...
export class WorldLighting {
  ...
  _sky() {
    ...
    // Fallback static texture if video hasn't loaded or fails
    const fallbackTex = new THREE.Texture(); // Or load a small placeholder
    
    // Fetch VideoTexture from our manager
    // We import videoManager dynamically or assume it's attached to window for now
    let skyTex;
    if (window.videoManager) {
      skyTex = window.videoManager.createVideoTexture('sky', 'https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4', true);
    }

    const skyMat = new THREE.MeshBasicMaterial({
      map: skyTex || fallbackTex,
      side: THREE.BackSide,
      fog: false,
      depthWrite: false
    });
