...
export class WorldLighting {
    ...
    const skyMat = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          // Stratospheric Ozone Chappuis absorption layer
          ...
          // --- 2. Golden Horizon Glow & Forward Scatter ---
          ...
          // --- 3. Ground Hemisphere Transition ---
          ...
          // --- 4. Radiant Solar Disc + Mie Aerosol Corona ---
  ...
  _composer() {
    ...
    const pr = this.world.renderer.getPixelRatio() || 1;
    const rw = Math.round(w * pr);
    const rh = Math.round(h * pr);
    
    // Create custom render target with 4x hardware MSAA for crystal-clear geometry edges without blur
    const renderTarget = new THREE.WebGLRenderTarget(rw, rh, {
      type: THREE.HalfFloatType,
      samples: 4,
    });
    renderTarget.depthTexture = new THREE.DepthTexture(rw, rh, THREE.FloatType);
    ...
    if (!isMobile) {
      // 1. High-Fidelity Optical Bloom Pass (clean specular solar glints without blurring scene)
      const bloom = new UnrealBloomPass(
        new THREE.Vector2(Math.ceil(rw / 2), Math.ceil(rh / 2)),
        0.16,   // strength
        0.45,   // radius
        0.88,   // threshold
      );
    ...
    // 2. Optical Clarity Vignette & Contrast Pass
    const CinematicLensShader = {
      uniforms: {
        tDiffuse: { value: null },
        uTime: { value: 0 },
        uResolution: { value: new THREE.Vector2(rw, rh) },
        uChromaticDispersion: { value: 0.0 }, // Zero chromatic aberration for razor-sharp edges
        uVignetteRoundness: { value: 0.85 },
        uVignetteDarkness: { value: 0.12 },
        uGrainIntensity: { value: 0.0 },     // Zero grain to eliminate pixel noise & flickering
      },
      vertexShader: `
        varying vec2 vUv;
        void main() {
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float vignette = 1.0 - smoothstep(0.45, 0.90, length(distFromCenter) * uVignetteRoundness) * uVignetteDarkness;
          col *= vignette;

          // Rich contrast grounding
