...
// ============================================================
import { WORLD, DISTRICTS, ROADS, RIVER_INLET, RIVER_OUTLET, terrainHeight } from './terrain.js?v=9';
  ...
  _renderBG() {
    ...
    for (let py = 0; py < bg.height; py += step) {
      for (let px = 0; px < bg.width; px += step) {
        ...
        let col;
        if (h < WORLD.waterLevel) col = '#4c8eb5';
        else if (wx < -220 && wz > 160 && h < 95) col = '#c87850'; // Grand Canyon & Desert Badlands terracotta rock
        else if (wz < -600 && Math.abs(wx) < 320) col = '#d9ccb8'; // Highland Cathedral Plateau sandstone
          ...
          else if (wz > 1400) col = '#4a8b54'; // Kaya Island emerald lushness
          else col = '#83a86e';
        }
        c.fillStyle = col;
    ...
    if (WORLD.mosque) {
      ...
      c.fillStyle = '#f8f4ec';
      c.fillRect(mx - 24 * devicePixelRatio, my - 32 * devicePixelRatio, 48 * devicePixelRatio, 64 * devicePixelRatio);
      c.fillStyle = '#189ab4';
      c.fillRect(mx - 10 * devicePixelRatio, my - 22 * devicePixelRatio, 20 * devicePixelRatio, 44 * devicePixelRatio);
    }

    // Buddhist Pagoda Courtyard
    if (WORLD.buddhistTemple) {
      const [px, py] = this.w2s(WORLD.buddhistTemple.x, WORLD.buddhistTemple.z);
      c.fillStyle = '#9e4736';
      c.beginPath();
      c.arc(px, py, 14 * devicePixelRatio, 0, Math.PI * 2);
      c.fill();
    ...
    c.lineCap = 'round';
    c.beginPath();
    c.moveTo(gx - 14 * dpr, gy + 10 * dpr); c.lineTo(gx - 14 * dpr, gy - 8 * dpr);
    c.moveTo(gx + 14 * dpr, gy + 10 * dpr); c.lineTo(gx + 14 * dpr, gy - 8 * dpr);
    c.moveTo(gx - 18 * dpr, gy - 8 * dpr); c.lineTo(gx + 18 * dpr, gy - 8 * dpr);
    c.stroke();
    c.fillStyle = '#2c2c34';
    c.font = `bold ${10 * dpr}px system-ui, sans-serif`;
    c.fillText('CELESTIAL GATE', gx, gy + 22 * dpr);
    ...
    const labels = {
      ...
      beach: [740, -200], summit: [-540, -450], desert: [-440, 360],
    ...
    for (const [k, [x, z]] of Object.entries(labels)) {
      ...
      const [sx, sy] = this.w2s(x, z);
      c.fillStyle = 'rgba(20,26,20,0.65)';
      const t = DISTRICTS[k].name.toUpperCase();
      c.fillText(t, sx, sy);
    }

    // Landmark labels & Buildings
    const [lx, ly] = this.w2s(WORLD.lake.x, WORLD.lake.z);
    c.fillStyle = 'rgba(255,255,255,0.85)';
    c.font = `italic 600 ${13 * devicePixelRatio}px Georgia, serif`;
    c.fillText('Mirror Lake', lx, ly);

    const [cx, cy] = this.w2s(WORLD.cathedral.x, WORLD.cathedral.z);
    c.fillStyle = '#d4af37';
    c.font = `bold ${12 * devicePixelRatio}px Georgia, serif`;
    c.fillText('🏰 Hohenzollern Fortress & Asamkirche', cx, cy - 20 * devicePixelRatio);

    const [wfx, wfy] = this.w2s(0, -460);
    c.fillStyle = '#6fa7c8';
    c.font = `600 ${11 * devicePixelRatio}px system-ui, sans-serif`;
    c.fillText('🌊 Great North Waterfall & Tarn', wfx, wfy);

    const [dx, dy] = this.w2s(-460, 340);
    c.fillStyle = '#b85d34';
    c.font = `bold ${11 * devicePixelRatio}px system-ui, sans-serif`;
    c.fillText('🏜️ Grand Canyon Pueblo Sanctuary', dx, dy);

    if (WORLD.mosque) {
      const [mx, my] = this.w2s(WORLD.mosque.x, WORLD.mosque.z);
      c.fillStyle = '#189ab4';
      c.font = `600 ${11 * devicePixelRatio}px system-ui, sans-serif`;
      c.fillText('🕌 Moorish Alhambra Palace', mx, my - 38 * devicePixelRatio);
    }
    ...
    if (WORLD.buddhistTemple) {
      ...
      c.fillStyle = '#9e4736';
      c.font = `600 ${11 * devicePixelRatio}px system-ui, sans-serif`;
      c.fillText('⛩️ Five-Tiered Zen Pagoda', px, py - 20 * devicePixelRatio);
    }

    const [kx, ky] = this.w2s(WORLD.kayaIsland.x, WORLD.kayaIsland.z);
    c.fillStyle = 'rgba(100,230,250,0.95)';
    c.font = `600 ${12 * devicePixelRatio}px Georgia, serif`;
    c.fillText('🐾 Guardian Husky Kaya Monument', kx, ky - 18 * devicePixelRatio);
