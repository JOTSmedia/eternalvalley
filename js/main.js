...
// ============================================================
import { generatePlots, DISTRICTS, PET_NAMES } from './terrain.js?v=26';
import { World3D } from './world3d.js?v=26';
import { Map2D } from './map2d.js?v=26';
import { Auth } from './auth.js?v=26';
import { State } from './state.js?v=26';
import { UI } from './ui.js?v=26';
import { SEASON_STYLE, MOODS } from './ambience.js?v=26';
import { EarthView } from './earth.js?v=26';
import { Theme } from './theme.js?v=26';
import { Atmosphere } from './atmosphere.js?v=26';
import { Motion } from './motion.js?v=26';
import { hydrate as hydrateIcons } from './icons.js?v=26';
import { warmThumbs, photosReady } from './thumbs.js?v=26';
import { CharityUI } from './charityui.js?v=26';
import { warmAllTextures } from './materials.js?v=26';
...
import { SLOTS, ITEM_DECOR, GIFT_DECOR, GIFTS, CHARITIES } from './catalog.js?v=26';
  ...
  enterPromise = (async () => {
    try {
      ...
      if (mode === '3d' || mode === 'tour') {
        try {
          if (mode === 'tour') {
            console.log('[enter] tour mode requested: launching Veo Drone Tour immediately');
            if (window.VeoTour) window.VeoTour.start(0);
            else if (window.veoTour) window.veoTour.start(0);
            UI.show3D('tour', false).catch(e => console.log('[enter] initial show3D tour call:', e));
          }
          ...
          console.log('[enter] startWorld resolved, world ready:', !!UI.world);
          if (mode === 'tour') {
            if (window.VeoTour) window.VeoTour.start(0);
            else if (window.veoTour) window.veoTour.start(0);
            if (UI.world?.setMode) UI.world.setMode('tour');
          } else {
            await UI.show3D(mode === '3d' ? 'orbit' : mode, false);
          }
        } catch (e) {
      ...
      try {
        ...
        if (params.get('tour') === 'true' || params.has('tour')) {
          setTimeout(() => {
        ...
        if (memId) {
          const { allMemorials } = await import('./social.js?v=26');
...
async function boot() { 
  ...
  if (!isProd) {
    ...
    window.RBV = {
      UI, Theme, get atmosphere() { return window.__evAtmosphere; }, plots, get earth() { return UI.earth; },
