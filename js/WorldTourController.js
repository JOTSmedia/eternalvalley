...
export class WorldTourController {
  ...
  _initTourSpline() {
    ...
    const tourPoints = [
      // 1. GATE - Monumental approach and direct flight through arch
      new THREE.Vector3(0, 48.0, 1300), 
      new THREE.Vector3(0, 36.5, 885),  // 36.5m clears the 56m crossbeam

      // 2. BRIDGE - Gliding along Grand Boulevard and soaring over bridge
      new THREE.Vector3(0, 42.0, 600), 
      new THREE.Vector3(0, 56.0, 300), 

      // 3. PLAZA - 360-degree orbit around the tiered lion fountain and tree
      new THREE.Vector3(80, 28.0, 50),
      new THREE.Vector3(-80, 26.0, -100),

      // 4. WATERFALL - Descending to mist level in clear air, ascending to crest
      new THREE.Vector3(0, 25.0, -300),  // mist level near plunge pool
      new THREE.Vector3(0, 155.0, -480), // soaring up to crest the 150m cliff lip

      // 5. TARN - Submerged dive exploring glowing boulders, resurfacing
      new THREE.Vector3(0, 168.0, -560),  // Submerged dive (tarn surface is ~174m)
      new THREE.Vector3(40, 185.0, -600), // Resurfacing cleanly

      // 6. CATHEDRAL - Flying straight between open Hohenzollern portals into Asamkirche nave
      new THREE.Vector3(0, 222.0, -620),  // Approaching the grand portal facade
      new THREE.Vector3(0, 224.0, -690),  // Penetrating the Asamkirche nave aisle toward high altar

      // 7. DESERT CANYON & PUEBLO SUN-KIVA - Sweeping across red rock badlands into the sun-kiva
      new THREE.Vector3(-320, 85.0, -850), // Soaring along the red rock rim canyons
      new THREE.Vector3(-460, 52.0, 260),  // Descending into Desert Bloom canyon wash
      new THREE.Vector3(-460, 44.5, 340),  // Entering Pueblo Sun-Kiva sanctuary interior, passing hearth and skylight

      // 8. MOSQUE - Gliding over the marble reflecting pool and Alhambra arcade into muqarnas dome
      new THREE.Vector3(-350, 160.0, -350), 
      new THREE.Vector3(-480, 104.0, -150), // Gliding intimately over 96m terrace

      // 9. LAKE - Skimming lake waters, diving underwater with koi
      new THREE.Vector3(0, 18.0, -180),     // Skimming valley meadows
      new THREE.Vector3(430, 8.0, -250),    // Underwater dive into Mirror Lake (surface is 12.5m)

      // 10. PAGODA - Flying past South Porch, spiraling up past eaves
      new THREE.Vector3(560, 138.0, -450), // Flying low past the porch (135m base)
      new THREE.Vector3(600, 240.0, -550), // Spiraling up the 5-tiered roof

      // 11. KAYA ISLAND - 360-degree orbit around Pure-Bred Siberian Husky Kaya Statue, plunging
      new THREE.Vector3(260, 180.0, 1200), // Soaring along coastal gorge
      new THREE.Vector3(20, 36.0, 2040),   // Gliding past pavilion & Pure-Bred Siberian Husky Kaya Monument
      new THREE.Vector3(-80, 14.0, 2180),  // Gliding past Cliff Temple of Baal, plunging into turquoise ocean reef

      // 12. PANORAMA - Radiant sunrise ocean breach for panoramic climb
      new THREE.Vector3(-100, 6.0, 1600),  // Ocean breach from underwater
      new THREE.Vector3(0, 75.0, 1200),    // Panoramic sunrise climb looping back to Gate
    ];
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    const POIs = [
      ...
      new THREE.Vector3(0, 224, -720),    // 6: Cathedral Altar
      new THREE.Vector3(-460, 44, 340),   // 7: Desert Canyon & Pueblo Sun-Kiva
      new THREE.Vector3(-480, 104, -200), // 8: Mosque
      new THREE.Vector3(400, 12, -270),   // 9: Lake
      new THREE.Vector3(560, 186, -540),  // 10: Pagoda
      new THREE.Vector3(0, 28, 2100),     // 11: Kaya Pure-Bred Siberian Husky Monument & Reef
      new THREE.Vector3(0, 75, 420),      // 12: Panorama
    ];
    ...
    // Hold the current POI for 70% of the stage, then rapidly smooth transition to the next
    const holdFrac = Math.max(0.0, (stageFrac - 0.7) / 0.3);
    const smooth = (x) => x * x * (3 - 2 * x);
    const blend = smooth(holdFrac);
    ...
    // Stages 1 & 2 are grand boulevard approaches (look ahead). 
    // Stage 12 is panorama climb (look ahead).
    // All other stages are orbits/dives/glides that demand strong POI tracking.
    const isStraightApproach = (stageIdx === 0 || stageIdx === 1 || stageIdx === 11);
    const lookAheadBlendWeight = isStraightApproach ? 0.65 : 0.15; // 85% lock to POI during orbits

    // Blend between looking ahead and looking at POI for sweeping cinematic feel
    target.copy(poiTarget).lerp(lookAhead, lookAheadBlendWeight);
