...
export class WorldTourController {
  ...
  _initTourSpline() {
    if (this._tourSpline && this._tourStages && this._stageArc && this._tourStages.length === 12) return;
    this._tourStages = DRONE_TOUR_LANDMARKS;

    const tourPoints = [
      // Stage 1: The Grand Triumphal Gate
      new THREE.Vector3(0, 48.0, 1300),   // Monumental approach
      new THREE.Vector3(0, 40.0, 1050),   // Approaching grand triumphal arch & ETERNAL VALLEY frieze
      new THREE.Vector3(0, 36.5, 885),    // Gliding through portal archway into sanctuary

      // Stage 2: The Rainbow Bridge Crest
      new THREE.Vector3(0, 42.0, 680),    // Grand Boulevard approach
      new THREE.Vector3(0, 52.0, 480),    // Ascending Rainbow Bridge ramp
      new THREE.Vector3(0, 58.0, 300),    // Cresting Rainbow Bridge arch above river

      // Stage 3: Central Plaza & Living Fountain
      new THREE.Vector3(80, 38.0, 120),   // Entering Plaza boulevard
      new THREE.Vector3(95, 36.0, 0),     // Sweeping orbit around Living Fountain & Lion basins
      new THREE.Vector3(-40, 36.0, -120), // Gliding past Sanctuary Tree & starburst mosaics

      // Stage 4: Cataract Waterfall Ascent
      new THREE.Vector3(0, 44.0, -280),   // Approaching cascade plunge pool mist
      new THREE.Vector3(0, 105.0, -420),  // Soaring vertically up the 182m waterfall face
      new THREE.Vector3(0, 168.0, -490),  // Cresting waterfall lip

      // Stage 5: High Glacial Tarn Underwater Dive
      new THREE.Vector3(0, 172.0, -540),  // Plunging beneath 182m tarn surface into clear glacial water
      new THREE.Vector3(25, 166.0, -575), // Deep cruise (16m depth) past glowing crystal boulders & trout pods
      new THREE.Vector3(15, 188.0, -605), // Resurfacing breach into crisp alpine air

      // Stage 6: Hohenzollern Cathedral & Asamkirche Nave
      new THREE.Vector3(0, 220.0, -630),  // Approaching Hohenzollern facade portal archway
      new THREE.Vector3(0, 218.0, -685),  // Gliding down central Asamkirche nave past Solomonic columns to altar
      new THREE.Vector3(0, 275.0, -735),  // Ascending up through open clerestory, clearing keep roof

      // Stage 7: Grand Canyon Desert (Northwest)
      new THREE.Vector3(-240, 288.0, -750),// Mountain pass flyover (clearance +38m above 250m ridge)
      new THREE.Vector3(-340, 238.0, -600),// Sweeping along Red Rock Canyon Rim overlooking badlands
      new THREE.Vector3(-380, 175.0, -420),// Gliding along canyon wash toward Pueblo Sun-Kiva sanctuary

      // Stage 8: Moorish Alhambra & Muqarnas Sanctuary (West)
      new THREE.Vector3(-450, 132.0, -300),// Ascending north garden terrace
      new THREE.Vector3(-480, 110.0, -200),// Gliding over marble reflecting pool terrace into arcade
      new THREE.Vector3(-380, 75.0, -120), // Gliding southeast from muqarnas sanctuary toward valley

      // Stage 9: Mirror Lake Realm & Underwater Dive (East)
      new THREE.Vector3(100, 30.0, -160),  // Crossing lake meadow shoreline beneath willows
      new THREE.Vector3(430, 2.8, -260),   // Deep underwater dive (9.7m depth) through 700+ golden koi & pedestals
      new THREE.Vector3(520, 25.0, -340),  // Resurfacing breach out of Mirror Lake into golden mist

      // Stage 10: Five-Tiered Zen Pagoda & Buddha Sanctuary (Northeast)
      new THREE.Vector3(530, 95.0, -420),  // Ascending Pagoda pine hillside past South Porch
      new THREE.Vector3(610, 175.0, -570), // Orbiting past golden Buddha and 5-tiered curved roofs
      new THREE.Vector3(630, 215.0, -470), // Spiraling up past bronze Sōrin finial, banking south

      // Stage 11: Kaya Island Pure-Bred Siberian Husky Monument (South)
      new THREE.Vector3(350, 215.0, 700),  // Soaring high down eastern coastal ridge, panoramic archipelago view
      new THREE.Vector3(20, 42.0, 2040),   // Sweeping frontal approach looking directly south at Siberian Husky Kaya Monument
      new THREE.Vector3(-30, 44.0, 2140),  // Sweeping past Cliff Temple of Baal over sea cliffs

      // Stage 12: Southern Ocean Coral Reef & Sunrise Panorama (Far South)
      new THREE.Vector3(10, -5.5, 2220),   // Plunging off sea cliffs into shallow sunlit coral garden (seabed -47m)
      new THREE.Vector3(-25, -14.5, 2290), // Deep dive into coral drop-off with branching staghorn corals & reef sharks (seabed -103m)
      new THREE.Vector3(0, 75.0, 1450),    // Radiant sunrise ocean breach & grand panoramic valley climb looping to Gate
    ];
    ...
    this._totalSplineLength = this._stageArc.reduce((a, b) => a + b, 0);
    this._tourTime = 0;
  ...
  _calculateTourLookTarget(t, pos, tangent, outTarget) {
    ...
    const POIs = [
      new THREE.Vector3(0, 40.0, 600),     // 1: Gate - Looking through Gate down Grand Boulevard
      new THREE.Vector3(0, 45.0, 100),     // 2: Bridge - Looking ahead over Bridge into Plaza
      new THREE.Vector3(0, 24.0, 0),       // 3: Plaza - Living Lion Fountain (orbit at R=80-95m)
      new THREE.Vector3(0, 160.0, -520),   // 4: Waterfall - Cataract Waterfall Crest
      new THREE.Vector3(20, 166.0, -620),  // 5: Tarn - Glacial Tarn Underwater Trout & Crystals
      new THREE.Vector3(0, 220.0, -720),   // 6: Cathedral - Asamkirche Nave Altar
      new THREE.Vector3(-440, 150.0, -500),// 7: Desert - Grand Canyon Badlands Panorama
      new THREE.Vector3(-480, 106.0, -200),// 8: Mosque - Moorish Alhambra Reflecting Pool
      new THREE.Vector3(470, 3.0, -290),   // 9: Lake - Mirror Lake Deep Golden Koi School (direct eye-level focus)
      new THREE.Vector3(560, 170.0, -520), // 10: Pagoda - Zen Pagoda Spire & Buddha (orbit at R=40-50m)
      new THREE.Vector3(20, 37.0, 2100),   // 11: Kaya - Pure-Bred Siberian Husky Kaya Statue Hero Focus
      new THREE.Vector3(0, -10.0, 2260),   // 12: Ocean - Coral Reef Marine Sanctuary
    ];
    ...
    const stageIdx = Math.floor(stageFloat) % 12;
    const nextIdx = (stageIdx + 1) % 12;
    const stageFrac = stageFloat - Math.floor(stageFloat);
    
    // Smooth quintic polynomial interpolation between POIs (C2 continuous: zero jerk at stage boundaries)
    const blend = stageFrac * stageFrac * stageFrac * (stageFrac * (stageFrac * 6 - 15) + 10);
    ...
    const lookAhead = this.world._v3Tmp3.copy(pos).addScaledVector(tan, 120.0);
    const poiTarget = this.world._v3Tmp4.copy(currentPOI).lerp(nextPOI, blend);
    
    const isOrbit = (stageIdx === 2 || stageIdx === 5 || stageIdx === 7 || stageIdx === 9 || stageIdx === 10);
    const lookAheadWeight = isOrbit ? 0.35 : 0.60;

    if (stageIdx === 10) {
      // Hero gaze: 85% focused on Kaya during frontal approach, gracefully handing off to lookAhead as drone flies past
      const kayaTarget = this.world._v3Tmp4.copy(POIs[10]); // (20, 37.0, 2100)
      const kayaWeight = Math.max(0.20, 0.88 - stageFrac * 0.95);
      const heroTarget = kayaTarget.lerp(lookAhead, 1.0 - kayaWeight);
      if (stageFrac < 0.55) {
        target.copy(heroTarget);
      } else {
        const exitK = (stageFrac - 0.55) / 0.45;
        const smoothExit = exitK * exitK * (3.0 - 2.0 * exitK);
        target.copy(heroTarget).lerp(POIs[11], smoothExit);
      }
    } else if (stageIdx === 8 && pos.y < 12.0) {
      target.copy(poiTarget).lerp(lookAhead, lookAheadWeight);
      target.y = 3.2; // Keep eye-level gaze directly through swimming koi school
    } else {
      target.copy(poiTarget).lerp(lookAhead, lookAheadWeight);
    }

    // Steadicam Gimbal Anti-Lock Singularity Guard (guaranteed minimum 5m focal clearance)
    const distSq = target.distanceToSquared(pos);
    if (distSq < 25.0) {
      target.copy(pos).addScaledVector(tan, 50.0);
    }
  ...
  _initWalkHUD() {
    ...
    if (!pill) {
      ...
      pill.innerHTML = `
        ...
        <button class="swp-btn is-active" data-cam-mode="tour">🛸 3D Drone Flight</button>
        <button class="swp-btn" data-cam-mode="film">🎬 Film Tour</button>
      ...
      pill.addEventListener('click', (e) => {
        ...
        if (!btn) return;
        if (btn.dataset.camMode === 'film') {
          if (window.UI?.show3D) {
            window.UI.show3D('film');
          } else {
  ...
  toggleTourPlayPause() {
    ...
    this._updateTourHUD(this._tourTime, true);
  }
  ...
  _updateTourHUD(t, force = false) {
    // If photorealistic Veo drone tour is active, suppress legacy low-poly HUD
    const veoActive = window.VeoTour?.isPlaying || (document.getElementById('veoDroneTourContainer') && !document.getElementById('veoDroneTourContainer').classList.contains('hidden'));
    if (veoActive) {
    ...
    if (this._lastStageNum !== activeStage.stage || !card || force) {
      ...
      const totalStages = this._tourStages ? this._tourStages.length : 12;
      const pauseHint = this.world._tourPaused
        ? '<div class="dtc-pause-hint">⏸ <strong>Flight Paused</strong> — Click any plot to inspect & reserve · Press Space to resume</div>'
        : '';

      card.innerHTML = `
        <div class="dtc-content">
          <div class="dtc-progress-wrap"><div class="dtc-progress" id="dtcProgress"></div></div>
          <span class="dtc-title"><span class="dtc-stage">${activeStage.stage}/${totalStages}</span> ${activeStage.title}</span>
          ${pauseHint}
      ...
      const playBtn = card.querySelector('.dtc-play');
      if (playBtn) playBtn.textContent = this.world._tourPaused ? '▶' : '⏸';
      let hintEl = card.querySelector('.dtc-pause-hint');
      if (this.world._tourPaused) {
        if (!hintEl) {
          const content = card.querySelector('.dtc-content');
          if (content) {
            hintEl = document.createElement('div');
            hintEl.className = 'dtc-pause-hint';
            hintEl.innerHTML = '⏸ <strong>Flight Paused</strong> — Click any plot to inspect & reserve · Press Space to resume';
            content.insertBefore(hintEl, content.querySelector('.dtc-controls'));
          }
        }
      } else {
  ...
  _updateWalk(dt) {
    ...
    if (this.tourMode) {
      ...
      const totalStages = this._tourStages ? this._tourStages.length : 12;
      const stageFracPerSec = (1.0 / totalStages) / (stageSeconds || 12);
      ...
      // Dynamic POI Camera Look Target (strictly forward for Legs 1 & 2, rotating & tracking POI objects for Legs 3-12)
      this._calculateTourLookTarget(t, this.world._v3TourPos, this.world._v3TourTan, this.world._v3TourLook);
      ...
      const isStraightLeg = (t <= (2.0 / totalStages));
  ...
  flyToDistrict(key, plot) {
    ...
    const centers = {
      ...
      starlight: { x: 20, y: 36, z: 2100, dist: 220 },
      beach: { x: 20, y: 36, z: 2100, dist: 220 },
      kaya_island: { x: 20, y: 36, z: 2100, dist: 220 },
      highland: { x: 0, y: 220, z: -680, dist: 220 },
      summit: { x: -360, y: 92, z: -380, dist: 220 },
      highland_sanctuary: { x: 0, y: 220, z: -680, dist: 220 },
      all: { x: 0, y: 160, z: 720, dist: 380 },
      overview: { x: 0, y: 160, z: 720, dist: 380 },
      desert: { x: -460, y: 42, z: 340, dist: 220 },
      underwater: { x: 380, y: -2, z: -250, dist: 120 },
      desert_bloom: { x: -460, y: 42, z: 340, dist: 220 },
      mosque: { x: -480, y: 104, z: -200, dist: 170 },
      pagoda: { x: 560, y: 140, z: -540, dist: 180 },
      waterfall: { x: 0, y: 95, z: -460, dist: 180 },
      lake: { x: 380, y: 16, z: -250, dist: 190 },
      bridge: { x: 0, y: 14, z: 440, dist: 150 },
      gate: { x: 0, y: 32, z: 880, dist: 160 },
      cathedral: { x: (typeof WORLD !== 'undefined' && WORLD.cathedral ? WORLD.cathedral.x : 0), y: 225, z: (typeof WORLD !== 'undefined' && WORLD.cathedral ? WORLD.cathedral.z : -687), dist: 160 },
      cathedral_interior: { x: (typeof WORLD !== 'undefined' && WORLD.cathedral ? WORLD.cathedral.x : 0), y: 216, z: (typeof WORLD !== 'undefined' && WORLD.cathedral ? WORLD.cathedral.z - 10 : -697), dist: 35 },
      desert_interior: { x: -460, y: 45, z: 340, dist: 28 },
      mosque_interior: { x: (typeof WORLD !== 'undefined' && WORLD.mosque ? WORLD.mosque.x : -480), y: 110, z: (typeof WORLD !== 'undefined' && WORLD.mosque ? WORLD.mosque.z - 10 : -200), dist: 30 },
      pagoda_interior: { x: (typeof WORLD !== 'undefined' && WORLD.buddhistTemple ? WORLD.buddhistTemple.x : 560), y: 144, z: (typeof WORLD !== 'undefined' && WORLD.buddhistTemple ? WORLD.buddhistTemple.z - 10 : -540), dist: 30 },
    };
        if (key === 'underwater') {
      const target = new THREE.Vector3(380, -4, -250);
      const camPos = new THREE.Vector3(380, -2, -220);
      this._flyToExplicit(camPos, target, 1.4);
      return;
    }
    if (key === 'cathedral' || key === 'cathedral_exterior') {
      this.flyToCathedral('exterior');
      return;
    }
    if (key === 'cathedral_interior') {
      this.flyToCathedral('interior');
      return;
    }
    if (key === 'desert_interior') {
      const target = new THREE.Vector3(-460, 44, 335);
      const camPos = new THREE.Vector3(-460, 45, 355);
      this._flyToExplicit(camPos, target, 1.4);
      return;
    }
    if (key === 'mosque_interior') {
      const mx = typeof WORLD !== 'undefined' && WORLD.mosque ? WORLD.mosque.x : -480;
      const mz = typeof WORLD !== 'undefined' && WORLD.mosque ? WORLD.mosque.z : -200;
      const target = new THREE.Vector3(mx, 112, mz - 20);
      const camPos = new THREE.Vector3(mx, 110, mz + 16);
      this._flyToExplicit(camPos, target, 1.4);
      return;
    }
    if (key === 'pagoda_interior') {
      const px = typeof WORLD !== 'undefined' && WORLD.buddhistTemple ? WORLD.buddhistTemple.x : 560;
      const pz = typeof WORLD !== 'undefined' && WORLD.buddhistTemple ? WORLD.buddhistTemple.z : -540;
      const target = new THREE.Vector3(px, 146, pz - 20);
      const camPos = new THREE.Vector3(px, 144, pz + 16);
      this._flyToExplicit(camPos, target, 1.4);
      return;
    }
  ...
  flyToCathedral(view = 'exterior') {
    const cx = (typeof WORLD !== 'undefined' && WORLD.cathedral) ? WORLD.cathedral.x : 0;
    const cz = (typeof WORLD !== 'undefined' && WORLD.cathedral) ? WORLD.cathedral.z : -687;
    const cy = (typeof WORLD !== 'undefined' && WORLD.cathedral) ? WORLD.cathedral.y : 182;

    if (view === 'interior') {
      // Inside Asamkirche grand Rococo nave looking down central aisle towards the glowing golden high altar
      const target = new THREE.Vector3(cx, cy + 34, cz - 26);
      const camPos = new THREE.Vector3(cx, cy + 32, cz + 18);
      this._flyToExplicit(camPos, target, 1.4);
    } else {
      // Hohenzollern Fortress exterior - majestic 3/4 elevated perspective showcasing the castle battlements, keep, and spires
      const target = new THREE.Vector3(cx, cy + 42, cz - 10);
      const camPos = new THREE.Vector3(cx - 75, cy + 68, cz + 195);
      this._flyToExplicit(camPos, target, 1.4);
    }
  }

  _flyToExplicit(camPos, targetPos, secs = 1.4) {
    // Cancel conflicting entrance flight, tour mode, or walk mode so camera is never overwritten
    this._entranceFlight = null;
    this.walkMode = false;
    this.tourMode = false;
    if (this.world.controls) this.world.controls.enabled = true;
    if (this.world._flyTween) this.world._flyTween = null;
    ...
    this._flyTarget.copy(targetPos);
    this._flyEndP.copy(camPos);
