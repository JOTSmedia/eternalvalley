import { generateVeoVideo, generateOmniMemorial } from "./api.js";

// ============================================================
// SOMEWHERE OVER THE RAINBOW BRIDGE — UI: panels, modals, purchase & gift flows
// ============================================================
import { veoTour, VEO_LANDMARKS } from './VeoTourController.js';
import { DISTRICTS, SIZE_LABELS, PET_NAMES, SPECIES, EPITAPHS } from './terrain.js?v=26';
import { MEMBERSHIPS, PLOT_ITEMS, GIFTS, EARTH_PLOT, PHYSICAL_KEEPSAKES, SLOTS, HEADSTONE_STYLES, ITEM_DECOR, GIFT_DECOR, CHARITIES, GIFT_CHARITY_SHARE, charityName, fmtPrice } from './catalog.js?v=26';
import { CharityUI } from './charityui.js?v=26';
import { Tour } from './tour.js?v=26';
import { SPLITS, Campaigns, Ledger, fmt, charityById, toCents, split, calculateImpact } from './charity.js?v=26';
import { Auth } from './auth.js?v=26';
import { State } from './state.js?v=26';
import { checkout } from './checkout.js?v=26';
import { IS_DEMO, HAS_MAPS3D, IS_ADMIN } from './config.js?v=26';
import { RBV, allMemorials, allActivity, timeAgo } from './social.js?v=26';
import { Motion } from './motion.js?v=26';
import { icon, speciesIcon, speciesKey, rainbowMark, SPECIES_LABELS } from './icons.js?v=26';
import { thumbImg, canRender as canRenderThumb, photoFor } from './thumbs.js?v=26';
import { Soundscape } from './soundscape.js?v=26';
import { checkAnniversaries, generateICS, ANNIVERSARY_GIFTS } from './anniversary.js?v=26';
...
export const DISTRICT_FLY_MAP = {
  ...
  desert: 'desert',
  desert_bloom: 'desert',
  desert_interior: 'desert_interior',
  mosque: 'mosque',
  mosque_interior: 'mosque_interior',
  pagoda: 'pagoda',
  pagoda_interior: 'pagoda_interior',
  waterfall: 'waterfall',
  lake: 'lake',
  bridge: 'bridge',
  gate: 'gate',
  ...
  cathedral_interior: 'cathedral_interior',
};
...
function readOneFile(f, maxPx = 420) {
  return new Promise((resolve) => {
    if (!f || !(f instanceof Blob)) return resolve(null);
    // Limit to 10MB to avoid browser freeze
    if (f.size > 10 * 1024 * 1024) {
      console.warn('[photo] File exceeds 10MB limit');
      return resolve(null);
    }
    const img = new Image();
    ...
    img.onload = () => {
      try {
        ...
        if (typeof UI !== 'undefined' && UI.toast) UI.toast('Failed to load the image. Please try another one.', 'warning');
        URL.revokeObjectURL(url);
        resolve(null);
      }
    };
...
function readPhoto(fileInput, maxPx = 420) {
...
export const UI = {
  ...
  init({ earth, plots, ensureWorld }) {
    ...
    $('#districtNav')?.addEventListener('click', async (e) => {
      ...
      if (d) {
        const districtNav = $('#districtNav');
        if (districtNav) {
            districtNav.querySelectorAll('.district-btn').forEach(b => b.classList.remove('is-active'));
        }
        btn.classList.add('is-active');
        await this.show3D('orbit');
        if (d === 'cathedral' || d === 'cathedral_exterior') {
          this.world?.flyToCathedral('exterior');
        } else if (d === 'cathedral_interior') {
          this.world?.flyToCathedral('interior');
        } else {
    ...
    $('#sanctuaryAmbiencePill')?.addEventListener('click', (e) => {
      ...
      if (!btn) return;
      const pill = $('#sanctuaryAmbiencePill');
      if (pill) {
    ...
    // 3D Cathedral Navigation Pill (Fortress Exterior vs Rococo Interior)
    $('#cathExteriorBtn')?.addEventListener('click', async () => {
      $('#cathExteriorBtn').classList.add('btn-gold');
      $('#cathExteriorBtn').classList.remove('btn-outline');
      $('#cathInteriorBtn')?.classList.add('btn-outline');
      $('#cathInteriorBtn')?.classList.remove('btn-gold');
      await this.show3D('orbit');
      this.world?.flyToCathedral('exterior');
    });

    $('#cathInteriorBtn')?.addEventListener('click', async () => {
      $('#cathInteriorBtn').classList.add('btn-gold');
      $('#cathInteriorBtn').classList.remove('btn-outline');
      $('#cathExteriorBtn')?.classList.add('btn-outline');
      $('#cathExteriorBtn')?.classList.remove('btn-gold');
      await this.show3D('orbit');
      this.world?.flyToCathedral('interior');
    });
    ...
    const toggleDroneTour = () => {
      const isVeoActive = veoTour.isPlaying || (veoTour.container && !veoTour.container.classList.contains('hidden'));
      if (isVeoActive || this.world?.tourMode) {
        veoTour.stop();
        if (this.world?.setMode) this.world.setMode('orbit');
    ...
    if ($('#cathedralToolbarBtn')) $('#cathedralToolbarBtn').onclick = async () => {
      await this.show3D('orbit');
      this.world?.flyToCathedral('exterior');
    };
  ...
  async showGlobe() {
    if (!this.globe) {
      if (this._globePromise) {
        await this._globePromise;
      } else {
        this._globePromise = (async () => {
          const { Globe } = await import('./globe.js?v=26');
        ...
        await this._globePromise;
        this._globePromise = null;
      }
    }
    this._setView({ view: 'viewGlobe', btn: 'btnGlobe' });
    requestAnimationFrame(() => { 
      if (this._currentView === 'viewGlobe') {
        this.globe?.resize(); 
        this.globe?.start(); 
      }
    });
  ...
  async descendTo(pin) {
    if (this._descendTimer) {
      clearTimeout(this._descendTimer);
      this._descendTimer = null;
    }
    if (pin.rbv) {
    ...
    if (this._currentView === 'viewGlobe') {
      ...
      if (pin.memorial) {
        this._descendTimer = setTimeout(() => {
          if (this._currentView === 'viewEarth') this.openEarthMemorial(pin.memorial);
        }, 2600);
      }
    }
  },
  ...
  _setView(which) {
    ...
    const cathPill = $('#cathedralNavPill');
    if (cathPill) {
      cathPill.classList.toggle('hidden', which.view !== 'view3d');
    }
    const showLegend = which.view === 'view2d';
    const legendEl = document.querySelector('.legend');
    if (legendEl) legendEl.classList.toggle('hidden', !showLegend);
  ...
  async showEarth() {
    ...
    if (!this._earthMounted) {
      if (this._earthMountPromise) {
        await this._earthMountPromise;
      } else {
        this._earthMountPromise = (async () => {
      ...
      setTimeout(() => {
        if (this._currentView === 'viewEarth' && this.earth?.leaflet) {
  ...
  async ensureWorld() {
    ...
    if (this._ensureWorldPromise) return this._ensureWorldPromise;

    this._ensureWorldPromise = (async () => {
      try {
        ...
        this.toast('Failed to load the Sanctuary 3D world. Please refresh the page.', 'warning');
      } finally {
        this._ensureWorldPromise = null;
      }
      return this.world || window.world;
    })();

    return this._ensureWorldPromise;
  },
  ...
  async show3D(mode = 'tour', isEntrance = false) {
    ...
    // When in 3D modes (tour, orbit, walk), hide legacy video overlay and pause video
    const landing = document.getElementById('veoLanding');
    if (landing && landing.style.display !== 'none') {
      landing.style.display = 'none';
      const video = document.getElementById('veoVideo');
      if (video) video.pause();
      this._veoActive = false;
    }
    const stage = $('#stage');
    if (stage) {
      stage.classList.remove('hidden');
    ...
    // Instantly launch or stop Veo Drone Tour so user has zero wait time
    if (mode === 'tour') {
      veoTour.start(0);
    } else {
      veoTour.stop();
    ...
    if (!this.world) {
      if (mode !== 'tour') this.toast('Loading the Sanctuary…', 2000, 'sparkle');
      return;
    }
    ...
    if (isEntrance && this.world.startEntranceFlight) {
      this.world.startEntranceFlight({
        ...
        duration: 7.0,
        onThresholdCross: () => {
          if (this._currentView !== 'view3d') return;
          // Synchronize Soundscape and Welcoming Solace
          Soundscape.init();
          Soundscape.playBowlGong(216);
          Soundscape.playChime(528, 0.12);
          setTimeout(() => { if (this._currentView === 'view3d') Soundscape.playChime(660, 0.08); }, 350);
          setTimeout(() => { if (this._currentView === 'view3d') Soundscape.playChime(880, 0.06); }, 700);
          Soundscape.setMode('crystal');
        },
        onComplete: () => {
          if (this._currentView !== 'view3d') return;
          this.toast('Welcome to Eternal Valley Sanctuary', 4500, 'rainbow');
          if (mode === 'tour') {
            veoTour.start(0);
            if (this.world?.setMode) this.world.setMode('tour');
      ...
      if (mode === 'tour') {
        ...
        veoTour.stop();
        if (this.world?.setMode) this.world.setMode(mode || 'orbit');
      }
    }

    if (this._currentView === 'view3d' && this.world.start) {
      this.world.start();
    }

    const triggerResize = () => {
      if (this._currentView === 'view3d') {
        this.world?._resize();
        this.world?.lighting?.applyAmbience();
      }
    };
  ...
  _setupVeoInteractiveHUD(landing, video) {
    if (this._veoHUDInitialized) {
      
      return;
    }
    this._veoHUDInitialized = true;

    const playPauseBtn = document.getElementById('veoPlayPauseBtn');
    const btnIcon = document.getElementById('veoBtnIcon');
    const flashEl = document.getElementById('veoPlayPauseFlash');
    const flashIcon = document.getElementById('veoPlayPauseIcon');
    const titleEl = document.getElementById('veoCurrentLandmarkName');
    const plotPins = document.getElementById('veoPlotPins');
    const plotCountEl = document.getElementById('veoPlotCount');
    const plotCard = document.getElementById('veoPlotCard');
    const vpcBadge = document.getElementById('vpcBadge');
    const vpcTitle = document.getElementById('vpcTitle');
    const vpcDesc = document.getElementById('vpcDesc');
    const vpcPrice = document.getElementById('vpcPrice');
    const vpcClose = document.getElementById('vpcClose');
    const vpcReserveBtn = document.getElementById('vpcReserveBtn');
    const vpcFly3dBtn = document.getElementById('vpcFly3dBtn');
    const progressBar = document.getElementById('veoProgressBar');
    const scrubberTrack = document.getElementById('veoScrubberTrack');
    const chapterTicks = document.getElementById('veoChapterTicks');
    const timeDisplay = document.getElementById('veoTimeDisplay');
    const muteBtn = document.getElementById('veoMuteBtn');
    const plotToggleBtn = document.getElementById('veoPlotToggleBtn');
    const enterBtn = document.getElementById('veoEnterBtn');

    let showPinsAlways = true;
    let currentSelectedPlot = null;
    let flashTimer = null;

    const VEO_CHAPTERS = [
      { id: 'gate', start: 0, end: 4, name: '1/14 · The Celestial Grand Gate', badge: 'SUNLIT MEADOWS', districtKey: 'meadows', plots: [
        { id: 'GATE_1', name: 'Triumphal Portal Garden #1', x: 28, y: 72, price: 249, badge: 'SUNLIT MEADOWS', desc: 'Serene flowering meadow bordering the celestial gate avenue, bathed in dawn sunlight.', flyKey: 'gate' },
        { id: 'GATE_2', name: 'Avenue of Remembrance #3', x: 50, y: 80, price: 280, badge: 'SUNLIT MEADOWS', desc: 'Prime garden sanctuary along the grand paved boulevard of arrival.', flyKey: 'gate' },
        { id: 'GATE_3', name: 'Golden Arch Vista Plot #5', x: 75, y: 70, price: 299, badge: 'SUNLIT MEADOWS', desc: 'Elevated plot commanding sweeping vistas toward the sunlit mountain range.', flyKey: 'gate' },
      ]},
      { id: 'bridge', start: 4, end: 8, name: '2/14 · The Crystalline Rainbow Bridge', badge: 'WHISPERING PINES', districtKey: 'woodland', plots: [
        { id: 'BRIDGE_1', name: 'Rainbow Gorge Crest #2', x: 30, y: 68, price: 299, badge: 'WHISPERING PINES', desc: 'Overlooks the rushing crystalline gorge and rainbow prism mist.', flyKey: 'bridge' },
        { id: 'BRIDGE_2', name: 'Crystal Stream Terrace #4', x: 65, y: 76, price: 349, badge: 'WHISPERING PINES', desc: 'Lush riverside plot shaded by ancient weeping willows and mountain pines.', flyKey: 'bridge' },
      ]},
      { id: 'plaza', start: 8, end: 12, name: '3/14 · Central Plaza & Tree of Life', badge: 'MEMORIAL MEADOWS', districtKey: 'meadows', plots: [
        { id: 'PLAZA_1', name: 'Tree of Life Canopy Plot #2', x: 44, y: 55, price: 399, badge: 'MEMORIAL MEADOWS', desc: 'Directly beneath the ancient sacred boughs of the glowing Tree of Life.', flyKey: 'meadows' },
        { id: 'PLAZA_2', name: 'Lion Fountain Promenade #5', x: 74, y: 68, price: 349, badge: 'MEMORIAL MEADOWS', desc: 'Beside the carved stone lion fountain with gentle splashing waters.', flyKey: 'meadows' },
      ]},
      { id: 'waterfall', start: 12, end: 16, name: '4/14 · Great North Waterfall & Mirror Lake', badge: 'LAKESIDE REST & RAPIDS', districtKey: 'lakefront', plots: [
        { id: 'WATERFALL_1', name: 'Mirror Lake Reflections #3', x: 32, y: 78, price: 449, badge: 'LAKESIDE REST', desc: 'Tranquil pebble shore where the cascading glacial waters meet the still mirror lake.', flyKey: 'lake' },
        { id: 'WATERFALL_2', name: 'Glacial Cataract Plunge #1', x: 55, y: 48, price: 499, badge: 'HIGHLAND RAPIDS', desc: 'Dramatic promontory directly beside the 182-meter thundering waterfall plunge pool.', flyKey: 'waterfall' },
        { id: 'WATERFALL_3', name: 'Cataract Mist Promontory #4', x: 76, y: 70, price: 549, badge: 'HIGHLAND RAPIDS', desc: 'Elevated terrace bathed in refreshing rainbow mist and mountain breezes.', flyKey: 'waterfall' },
      ]},
      { id: 'desert', start: 16, end: 20, name: '5/14 · Grand Canyon Desert Badlands [Exterior]', badge: 'DESERT BLOOM', districtKey: 'desert', plots: [
        { id: 'DESERT_1', name: 'Grand Canyon Sun-Sanctuary #1', x: 38, y: 72, price: 280, badge: 'DESERT BLOOM', desc: 'Warm terracotta sandstone mesa with flowering saguaro cacti and red rock spires.', flyKey: 'desert' },
        { id: 'DESERT_2', name: 'Pueblo Cliff-Dwelling Terrace #3', x: 68, y: 60, price: 320, badge: 'DESERT BLOOM', desc: 'Perched along the ancient cliff face below the pueblo stone sanctuary.', flyKey: 'desert' },
      ]},
      { id: 'pueblo_interior', start: 20, end: 24, name: '6/14 · Pueblo Sun-Kiva Sanctuary [Interior]', badge: 'ANCIENT PUEBLO SANCTUARY', districtKey: 'desert_interior', plots: [
        { id: 'PUEBLO_INT_1', name: 'Sun-Kiva Hearthside Plot #1', x: 32, y: 65, price: 360, badge: 'ANCIENT PUEBLO SANCTUARY', desc: 'Warm stone sanctuary beside the glowing ceremonial fire hearth and pine timber vigas.', flyKey: 'desert_interior' },
        { id: 'PUEBLO_INT_2', name: 'Celestial Skylight Altar Plot #2', x: 52, y: 52, price: 420, badge: 'ANCIENT PUEBLO SANCTUARY', desc: 'Directly beneath the natural circular rock skylight beam of golden morning sun.', flyKey: 'desert_interior' },
        { id: 'PUEBLO_INT_3', name: 'Canyon Overlook Portal Plot #3', x: 76, y: 58, price: 390, badge: 'ANCIENT PUEBLO SANCTUARY', desc: 'Adjacent to the dramatic stone arched overlook framing the Grand Canyon mesas.', flyKey: 'desert_interior' },
      ]},
      { id: 'cathedral_ext', start: 24, end: 28, name: '7/14 · Hohenzollern Fortress Cathedral [Exterior]', badge: 'HIGHLAND PLATEAU', districtKey: 'cathedral', plots: [
        { id: 'CATH_EXT_1', name: 'Hohenzollern North Rampart #1', x: 36, y: 68, price: 599, badge: 'HIGHLAND PLATEAU', desc: 'Commanding stone ramparts overlooking the valley and high alpine mountain crests.', flyKey: 'cathedral' },
        { id: 'CATH_EXT_2', name: 'Citadel Spire Terrace #4', x: 65, y: 72, price: 649, badge: 'HIGHLAND PLATEAU', desc: 'Exclusive terrace perched directly beneath the grand cylindrical castle keep.', flyKey: 'cathedral' },
      ]},
      { id: 'cathedral_int', start: 28, end: 32, name: '8/14 · Asamkirche Rococo Cathedral [Interior]', badge: 'ASAMKIRCHE ROCOCO', districtKey: 'cathedral_interior', plots: [
        { id: 'CATH_ROOF_1', name: 'Fresco Balcony West #1', x: 30, y: 55, price: 749, badge: 'ASAMKIRCHE ROCOCO', desc: 'Elevated interior balcony directly beneath the gilded ceiling fresco of the heavens.', flyKey: 'cathedral_interior' },
        { id: 'CATH_ROOF_2', name: 'Gilded Rococo Altar Chancel #2', x: 50, y: 62, price: 849, badge: 'ASAMKIRCHE ROCOCO', desc: 'Sacred sanctuary plot at the foot of the golden high altar and Solomonic columns.', flyKey: 'cathedral_interior' },
        { id: 'CATH_ROOF_3', name: 'Fresco Balcony East #3', x: 72, y: 54, price: 749, badge: 'ASAMKIRCHE ROCOCO', desc: 'Balcony sanctuary illuminated by divine god-rays pouring through upper clerestory windows.', flyKey: 'cathedral_interior' },
      ]},
      { id: 'moorish_ext', start: 32, end: 36, name: '9/14 · Moorish Alhambra Palace [Exterior]', badge: 'MOORISH OASIS', districtKey: 'mosque', plots: [
        { id: 'MOOR_EXT_1', name: 'Reflecting Pool Arcade #2', x: 42, y: 74, price: 480, badge: 'MOORISH OASIS', desc: 'Quiet colonnade plot reflecting the turquoise water and slender cypress trees.', flyKey: 'mosque' },
        { id: 'MOOR_EXT_2', name: 'Court of Lions Terrace #5', x: 68, y: 70, price: 520, badge: 'MOORISH OASIS', desc: 'Secluded garden terrace surrounded by fragrant jasmine and orange blossoms.', flyKey: 'mosque' },
      ]},
      { id: 'moorish_int', start: 36, end: 40, name: '10/14 · Moorish Palace Sanctuary [Interior]', badge: 'ALHAMBRA MUQARNAS', districtKey: 'mosque_interior', plots: [
        { id: 'MOOR_INT_1', name: 'Muqarnas Honeycomb Dome Plot #1', x: 35, y: 60, price: 560, badge: 'ALHAMBRA MUQARNAS', desc: 'Directly beneath the intricate vaulted muqarnas ceiling with golden sunlight shafts.', flyKey: 'mosque_interior' },
        { id: 'MOOR_INT_2', name: 'Marble Basin Fountain Plot #2', x: 50, y: 78, price: 590, badge: 'ALHAMBRA MUQARNAS', desc: 'At the edge of the central carved marble water basin with gentle bubbling waters.', flyKey: 'mosque_interior' },
        { id: 'MOOR_INT_3', name: 'Zellige Mosaic Colonnade #3', x: 70, y: 64, price: 540, badge: 'ALHAMBRA MUQARNAS', desc: 'Lapis lazuli mosaic arcade framing private contemplation alcoves.', flyKey: 'mosque_interior' },
      ]},
      { id: 'pagoda_ext', start: 40, end: 44, name: '11/14 · Five-Tiered Zen Pagoda & Lake [Exterior]', badge: 'PAGODA GARDENS', districtKey: 'pagoda', plots: [
        { id: 'PAG_EXT_1', name: 'Zen Pagoda Lotus Plot #1', x: 36, y: 76, price: 480, badge: 'PAGODA GARDENS', desc: 'Tranquil garden plot with weeping willows and floating cherry blossom petals.', flyKey: 'pagoda' },
        { id: 'PAG_EXT_2', name: 'Koi Terrace Waterfront #4', x: 66, y: 72, price: 540, badge: 'PAGODA GARDENS', desc: 'Waterside stone terrace where golden koi swim peacefully beneath the surface.', flyKey: 'pagoda' },
      ]},
      { id: 'pagoda_int', start: 44, end: 48, name: '12/14 · Zen Pagoda Sanctuary [Interior]', badge: 'ZEN BUDDHA SANCTUARY', districtKey: 'pagoda_interior', plots: [
        { id: 'PAG_INT_1', name: 'Golden Buddha Altar Plot #1', x: 34, y: 58, price: 590, badge: 'ZEN BUDDHA SANCTUARY', desc: 'Sacred tatami sanctuary in the warm glow of the golden Buddha and hanging lanterns.', flyKey: 'pagoda_interior' },
        { id: 'PAG_INT_2', name: 'Rock Garden View Pavilion #2', x: 68, y: 62, price: 620, badge: 'ZEN BUDDHA SANCTUARY', desc: 'Framed by open cedar fusuma doors looking out at the blooming sakura rock garden.', flyKey: 'pagoda_interior' },
      ]},
      { id: 'kaya_island', start: 48, end: 52, name: '13/14 · Kaya Island Coastal Sanctuary', badge: 'KAYA ISLAND', districtKey: 'starlight', plots: [
        { id: 'KAYA_1', name: 'Guardian Kaya Beacon Plot #1', x: 42, y: 65, price: 699, badge: 'KAYA ISLAND', desc: 'Beside the monumental crystalline beacon of guardian spirit Husky Kaya.', flyKey: 'starlight' },
        { id: 'KAYA_2', name: 'Azure Coral Shore #3', x: 65, y: 78, price: 649, badge: 'KAYA ISLAND', desc: 'Turquoise shoreline plot with gentle rolling surf and white sandy beaches.', flyKey: 'starlight' },
      ]},
      { id: 'panorama', start: 52, end: 56, name: '14/14 · Valley Summit Panorama', badge: 'SUMMIT REST', districtKey: 'highland', plots: [
        { id: 'SUMMIT_1', name: 'Celestial Ridge Sovereign Estate #1', x: 38, y: 66, price: 899, badge: 'SUMMIT REST', desc: 'High-altitude panoramic summit estate overlooking the entire interconnected sanctuary realm.', flyKey: 'highland' },
    ...
    const dismissVideo = (flyKey = null, targetMode = 'tour') => {
      if (!this._veoActive) return;
      this._veoActive = false;
      landing.style.transition = 'opacity 0.45s ease-out';
      landing.style.opacity = '0';
      this.show3D(targetMode, false);
      setTimeout(() => {
        landing.style.display = 'none';
        landing.style.opacity = '1';
        landing.style.transition = '';
        video.pause(); video.src = ""; video.load();
        if (targetMode === 'tour' && this.world?.startDroneTour) {
          this.world.startDroneTour(0);
        } else if (flyKey && this.world) {
          if (flyKey === 'cathedral_interior') {
            this.world.flyToCathedral?.('interior');
          } else if (flyKey === 'cathedral' || flyKey === 'cathedral_exterior') {
            this.world.flyToCathedral?.('exterior');
          } else {
            this.world.flyToDistrict?.(flyKey);
          }
        }
      }, 450);
    };
    ...
    const flashIndicator = (isPaused) => {
      if (!flashEl || !flashIcon) return;
      flashIcon.textContent = isPaused ? '⏸' : '▶';
    ...
    const togglePlayPause = () => {
      ...
      renderPinsForCurrentTime();
    };

    // Video click toggles play/pause (does not close video!)
    video.onclick = (e) => {
      e.stopPropagation();
      togglePlayPause();
    };
    ...
    window.addEventListener('keydown', (e) => {
      if (this._veoActive && e.code === 'Space') {
        e.preventDefault();
        togglePlayPause();
      }
    });
    ...
    if (muteBtn) {
      muteBtn.onclick = (e) => {
        ...
        muteBtn.textContent = video.muted ? '🔇 Sound Muted' : '🔊 Sound On';
      };
    }

    if (plotToggleBtn) {
      plotToggleBtn.onclick = (e) => {
        e.stopPropagation();
        showPinsAlways = !showPinsAlways;
        plotToggleBtn.style.opacity = showPinsAlways ? '1' : '0.6';
        renderPinsForCurrentTime();
      };
    }

    // Build scrubber chapter ticks
    if (chapterTicks && chapterTicks.children.length === 0) {
      VEO_CHAPTERS.forEach((ch) => {
        const tick = document.createElement('div');
        tick.style.position = 'absolute';
        tick.style.left = `${(ch.start / TOTAL_DURATION) * 100}%`;
        tick.style.top = '-3px';
        tick.style.width = '3px';
        tick.style.height = '12px';
        tick.style.background = 'rgba(212,175,55,0.75)';
    ...
    if (scrubberTrack) {
      scrubberTrack.onclick = (e) => {
        ...
        video.currentTime = clickRatio * TOTAL_DURATION;
      };
    }

    let lastChapterId = '';

    const renderPinsForCurrentTime = () => {
      if (!plotPins) return;
      const t = video.currentTime || 0;
      const curChapter = VEO_CHAPTERS.find(c => t >= c.start && t < c.end) || VEO_CHAPTERS[0];

      if (titleEl && curChapter.name !== titleEl.textContent) {
        titleEl.textContent = curChapter.name;
      }

      if (plotCountEl) {
        plotCountEl.textContent = curChapter.plots ? curChapter.plots.length : 0;
      }

      const shouldShowPins = showPinsAlways || video.paused;
      plotPins.style.display = shouldShowPins ? 'block' : 'none';
      plotPins.style.pointerEvents = shouldShowPins ? 'auto' : 'none';

      if (curChapter.id === lastChapterId && plotPins.children.length > 0) return;
      lastChapterId = curChapter.id;

      plotPins.innerHTML = '';
      if (!curChapter.plots) return;

      curChapter.plots.forEach(plot => {
        ...
        pin.style.zIndex = '12';
        pin.style.transition = 'transform 0.2s cubic-bezier(0.2,0.8,0.2,1)';
        ...
        pin.onmouseenter = () => {
          pin.style.transform = 'translate(-50%, -108%) scale(1.1)';
        };
        pin.onmouseleave = () => {
          pin.style.transform = 'translate(-50%, -100%) scale(1.0)';
        };
        ...
        pin.onclick = (e) => {
          ...
          if (plotCard) {
            if (vpcBadge) vpcBadge.textContent = plot.badge;
            if (vpcTitle) vpcTitle.textContent = plot.name;
            if (vpcDesc) vpcDesc.textContent = plot.desc;
            if (vpcPrice) vpcPrice.textContent = `$${plot.price} · Consecrated`;
            plotCard.style.display = 'block';
          }
        };

        plotPins.appendChild(pin);
      });
    };
    ...
    if (vpcReserveBtn) {
      vpcReserveBtn.onclick = (e) => {
        e.stopPropagation();
        const plot = currentSelectedPlot;
        this.show3D('tour');
        setTimeout(() => {
          this.griefWizardModal(plot);
        }, 600);
      };
    }

    if (vpcFly3dBtn) {
      vpcFly3dBtn.onclick = (e) => {
        e.stopPropagation();
        dismissVideo(currentSelectedPlot ? currentSelectedPlot.flyKey : null, 'orbit');
      };
    }

    video.ontimeupdate = () => {
      const cur = video.currentTime || 0;
      const pct = Math.min(100, (cur / TOTAL_DURATION) * 100);
      if (progressBar) progressBar.style.width = `${pct}%`;

      if (timeDisplay) {
        const curM = Math.floor(cur / 60).toString().padStart(2, '0');
        const curS = Math.floor(cur % 60).toString().padStart(2, '0');
        timeDisplay.textContent = `${curM}:${curS} / 00:56`;
      }
  ...
  /**
   * Launch the Hollywood-grade Cinematic Drone Tour across Eternal Valley
   */
  async startDroneTour(stageIndex = 0) {
    await this.show3D('tour');
    veoTour.start(stageIndex);
    if (this.world?.setMode) {
      this.world.setMode('tour');
    }
  },
  async show2D() {
    if (!this.map) {
      if (window.UI?.map) this.map = window.UI.map;
      else {
        if (this._ensureWorld) await this.ensureWorld();
        if (!this.map) {
          try {
            const { Map2D } = await import('./map2d.js?v=26');
            const c2d = document.getElementById('canvas2d') || document.querySelector('canvas#canvas2d');
            if (c2d) {
              const plotsList = (this.plots && this.plots.length) ? this.plots : (window.plots || this.world?.plots || []);
              this.map = new Map2D(c2d, plotsList, (p) => {
                this.openPlot(p);
                this.world?.selectPlot(p);
              });
            }
          } catch (e) {
            console.error('[show2D] Error creating Map2D:', e);
            this.toast('Failed to load the 2D map.', 'warning');
          }
        }
      }
    }
  ...
  openPlot(plot) {
    ...
    if (plot.status === 'available') {
      ...
      body.innerHTML = `
        ...
        ${owned ? `<button class="btn btn-gold btn-block" id="pCampaignBtn" style="margin-top:12px">${icon('heart')} Start a Fundraising Campaign</button>` : ''}
      ...
      if (owned) {
        $('#pCampaignBtn').onclick = () => {
          this.closePanel();
          CharityUI.createModal(this);
          // Pre-fill the modal with memorial data
          setTimeout(() => {
            const mName = document.getElementById('cmpName');
            const mSpecies = document.getElementById('cmpSpecies');
            const mYears = document.getElementById('cmpYears');
            if (mName) mName.value = m.petName || '';
            if (mSpecies) mSpecies.value = m.species || '';
            if (mYears) mYears.value = m.years || '';
          }, 50);
        };
      }
      $('#pShareBtn').onclick = () => {
    ...
    const vBtn = $('#ppbViewVision');
    if (vBtn) vBtn.onclick = () => veoTour.openAreaInspection(lm.id, plot.id);
    const fBtn = $('#ppbFlyTour');
    if (fBtn) fBtn.onclick = async () => {
      this.closePanel();
      await this.startDroneTour(lm.id);
    };
  ...
  authModal(afterLogin) {
    if (IS_DEMO) {
      ...
      $('#pDemoIn').onclick = () => {
        const n = $('#dName').value.trim() || 'Demo User';
        Auth._demoLogin(n, 'demo@local', 'local');
        done(`Welcome, ${n}.`);
      };
      $('#pGuest').onclick = () => { Auth.continueAsGuest(true); done('Browsing anonymously.'); };
      return;
    }
    ...
    this.modal(`
      ...
      <p class="fine">Secured by Firebase Authentication.</p>`);
    const done = (msg) => { this.closeModal(); this.toast(msg); afterLogin?.(); };
  ...
  petProfileHTML(m, id, own) {
    ...
    return `
      ...
      ${pp.gallery?.length ? `<div class="gallery-grid">${pp.gallery.map(g => `<img src="${g}" alt="Memorial Gallery Photo" loading="lazy">`).join('')}</div>` : ''}
      ...
      ${memories.length ? `<div class="sub" style="margin-top:14px">Memory wall (${memories.length}):</div>` + memories.slice(0, 8).map(x => `
        <div class="memory-card">
          ${x.photo ? `<img src="${x.photo}" alt="User Photo">` : ''}
  ...
  memorialCertificateModal(item) {
    ...
    const locationStr = isPlot 
      ? `Eternal Valley Sanctuary · ${DISTRICTS[item.district]?.name || 'Memorial Grove'} · Plot ${item.id}`
      ...
      <div class="cert-modal-wrap" id="certModalWrap">
        <div class="cert-card" id="certCardPrint">
          ...
          <div class="cert-header">
            ...
            <div class="cert-sub">ETERNAL VALLEY · SOMEWHERE OVER THE RAINBOW BRIDGE</div>
          </div>
  ...
  // ============================================================
  //  CAMPAIGN FLOWS (Memorial Fundraising)
  // ============================================================
  campaignCreateModal() {
    if (!Auth.user || Auth.user.isGuest) {
      return this.authModal(() => this.campaignCreateModal());
    }
    const tpl = $('#tpl-campaign-create').innerHTML;
    this.modal(tpl);
    
    const box = $('#modalBox');
    const sel = box.querySelector('#cmpCharity');
    sel.innerHTML = CHARITIES.map(c => `<option value="${c.id}">${esc(c.name)} — ${esc(c.cat)} (${esc(c.city)}, ${esc(c.state)})</option>`).join('');
    ...
    box.querySelector('#cmpCreate').onclick = () => {
      ...
      if (!name) return this.toast('Please enter their name.', 'warning');
      const c = Campaigns.create({
        petName: cleanText(name),
        species: cleanText(box.querySelector('#cmpSpecies').value.trim()),
        years: cleanText(box.querySelector('#cmpYears').value.trim()),
        story: cleanText(box.querySelector('#cmpStory').value.trim()),
        charityId: sel.value,
        goalCents: +box.querySelector('#cmpGoal').value,
        owner: Auth.user.name || 'A loving friend',
      });
      this.closeModal();
      CharityUI.renderPanel(this);
      $('#campaignPanel')?.classList.remove('hidden');
      this.toast(`${c.petName}’s campaign is live.`, 'heart');
      this.campaignViewModal(c);
    };
  },

  campaignViewModal(c) {
    const tpl = $('#tpl-campaign-view').innerHTML;
    this.modal(tpl);
    const box = $('#modalBox');
    ...
    box.querySelector('#cvTitle').innerHTML = `${icon('crest')} ${esc(c.petName)}`;
    box.querySelector('#cvSub').textContent = `${[c.species, c.years].filter(Boolean).map(s => esc(s)).join(' · ') || 'In loving memory'} · by ${esc(c.owner || 'A loving family')}`;
    
    const storyEl = box.querySelector('#cvStory');
    if (c.story) storyEl.innerHTML = `“${esc(c.story)}”`; else storyEl.style.display = 'none';

    box.querySelector('#cvProgress').style.width = `${(p * 100).toFixed(1)}%`;
    box.querySelector('#cvRaised').textContent = fmt(raised);
    box.querySelector('#cvGoal').textContent = fmt(c.goalCents);
    box.querySelector('#cvGiversCount').textContent = gifts.length;
    box.querySelector('#cvGiversLabel').textContent = gifts.length === 1 ? 'Tribute Giver' : 'Tribute Givers';

    const blurb = box.querySelector('#cvCharityBlurb');
    if (ch) {
      blurb.innerHTML = `
    ...
    const giftsList = box.querySelector('#cvGiftsList');
    if (gifts.length) {
      const when = (t) => new Date(t).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' });
      giftsList.innerHTML = `<div class="shop-cat" style="margin-top:18px">Memorial Tributes &amp; Givers</div>` +
        gifts.map(d => `
          <div class="feed-item">
    ...
    box.querySelector('#cvGive').onclick = () => this.campaignDonateModal(c);
    box.querySelector('#cvShare').onclick = () => {
      navigator.clipboard?.writeText(shareUrl);
      this.toast('Campaign tribute link copied to clipboard!', 'share');
    };
  },

  campaignDonateModal(c) {
    const tpl = $('#tpl-campaign-donate').innerHTML;
    this.modal(tpl);
    const box = $('#modalBox');
    ...
    box.querySelector('#cdTitle').innerHTML = `${icon('heart')} Give in ${esc(c.petName)}’s name`;
    box.querySelector('#cdSub').innerHTML = `Supporting <b>${esc(ch?.name || 'the rescue charity')}</b>. <b>100% of your donation passes directly to the shelter.</b>`;

    box.querySelector('#cdTiers').innerHTML = tiers.map((t, i) => `
    ...
    const paint = () => {
      ...
      const tierNote = box.querySelector('#cdTierNote');
      if (tierNote) {
        tierNote.innerHTML = activeTier
          ? `✦ <b>Your impact:</b> ${esc(activeTier.desc)}`
          : `✦ <b>Your impact:</b> 100% directly funds vital animal rescue operations & veterinary care.`;
      }
      
      const breakBox = box.querySelector('#cdBreak');
      if (breakBox) {
      ...
      const b = box.querySelector('#cdGo');
      if (b) b.innerHTML = `${icon('heart')} Complete Tribute Donation (${fmt(s.gross)})`;
    };
    paint();
    ...
    box.querySelectorAll('[data-amt]').forEach(b => {
      b.onclick = () => {
        ...
        box.querySelector('#cdOther').value = '';
        cents = +b.dataset.amt;
        paint();
      };
    });
    box.querySelector('#cdOther').oninput = (e) => {
    ...
    box.querySelector('#cdName').placeholder = esc(Auth.user?.name || 'A caring friend');

    box.querySelector('#cdGo').onclick = async (e) => {
      const btn = e.currentTarget;
      if (cents < 100) return this.toast('Minimum donation is $1.00.', 'warning');
      const donor = cleanText(box.querySelector('#cdName').value.trim()) || Auth.user?.name || 'Anonymous';
      const message = cleanText(box.querySelector('#cdMsg').value.trim());
      ...
      try {
        ...
        this.closeModal();
        CharityUI.renderPanel(this);
        this.toast(`${fmt(entry?.charity ?? cents)} delivered to ${charityName(c.charityId)} — thank you!`, 5500, 'heart');
        CharityUI.thanksModal(this, c, entry);
      } catch (err) {
        this.toast(String(err.message), 'warning');
        btn.disabled = false; btn.style.opacity = 1; btn.innerHTML = `${icon('heart')} Complete Tribute Donation`;
      }
    };
  },

  // ============================================================
  //  MEMORIAL CONSECRATION WIZARD — guided 5-step journey
  ...
  griefWizardModal(initialType = 'sanctuary', presetData = null) {
    ...
    let currentPhotoDataUrl = data.photo || null;
    ...
    const districtCards = Object.entries(DISTRICTS).map(([k, d]) => {
      ...
      return `
      <div class="gw-district-card ${k === initialDistrict ? 'is-selected' : ''}" data-district="${k}" data-price="${d.base}">
        ...
        <div class="gw-district-info">
          ...
          <small style="display:block;margin-top:3px;color:var(--accent-hi-c);font-size:11px">
            ${availCount > 0 ? `${availCount} plots available · ` : ''}Plot Tier: $${d.base}
          </small>
        </div>
      </div>`;
    }).join('');
    ...
    const charityCards = CHARITIES.map((c) =>
      `<div class="gw-charity-card ${c.id === (data.charity || CHARITIES[0]?.id) ? 'is-selected' : ''}" data-charity="${c.id}">
        ...
        <div class="gw-charity-desc">${c.blurb?.slice(0, 90) || c.mission?.slice(0, 90) || 'Helping animals in need'}…</div>
        <small class="fine-dim" style="display:block;margin-top:3px">EIN: ${c.ein} · ${c.city}, ${c.state}</small>
      </div>`
    ).join('');
    ...
    this.modal(`
      <div class="gw-wrap" id="createMemorialModal" role="region" aria-label="Memorial Consecration Guided Journey">
        ...
        <div class="gw-step is-active" data-step="1">
          ...
          <input id="gwName" placeholder="e.g. Luna, Max, Biscuit…" maxlength="30" value="${esc(data.name || '')}" autofocus required>
          ...
          <div class="two-col" style="margin-top:6px">
            <div>
              ...
              <input id="gwBirthYear" placeholder="e.g. 2012" maxlength="16" value="${esc(data.birthYear || '')}">
            </div>
            <div>
              <label>Passing Year / Date</label>
              <input id="gwPassYear" placeholder="e.g. 2025" maxlength="16" value="${esc(data.passYear || '')}">
            </div>
          </div>
          <input type="hidden" id="gwYears" value="${esc(data.years || '')}">
        ...
        <div class="gw-step" data-step="2">
          <div class="gw-step-icon">${icon('photo', { size: 38 })}</div>
          ...
          <div class="gw-photo-box">
            <div class="gw-photo-avatar" id="gwPhotoPreview">
              ${currentPhotoDataUrl ? `<img src="${currentPhotoDataUrl}" alt="Companion preview">` : speciesIcon(initialSpecies, { size: 46 })}
            </div>
            <div class="gw-photo-status" id="gwPhotoStatus">
              ${currentPhotoDataUrl ? 'Portrait ready for consecration' : 'No photo uploaded yet (optional)'}
            </div>
            <div class="gw-photo-actions">
              <input id="gwPhoto" type="file" accept="image/*" style="display:none">
              <button type="button" class="btn btn-outline" id="gwChoosePhotoBtn">${icon('photo')} Choose Photo</button>
              <button type="button" class="btn btn-outline ${currentPhotoDataUrl ? '' : 'hidden'}" id="gwRemovePhotoBtn" style="color:#f87171">${icon('close')} Remove</button>
            </div>
            <small class="fine-dim" style="font-size:11px">Supports JPG, PNG, WebP up to 10MB · Automatically optimized</small>
          </div>
        ...
        <div class="gw-step" data-step="3">
          ...
          <textarea id="gwEpitaph" rows="3" maxlength="140" placeholder="Forever chasing butterflies in the sunlight…">${esc(data.epitaph || '')}</textarea>
          ...
          <div class="gw-prompts" title="Click to fill comfort words">
            ...
            <span class="gw-prompt" data-txt="Rest peacefully in eternal sunshine.">🕊️ Peace</span>
        ...
        <div class="gw-step" data-step="5">
          ...
          <p class="gw-step-sub">${Math.round(SPLITS.plot.charity * 100)}% of your consecrated plot goes directly to the verified animal rescue you choose.</p>
          ...
          <button class="btn btn-gold btn-block btn-lg" id="gwCreateBtn">
            ${icon('crest')} Consecrate Their Memorial
          </button>
          <p class="fine" style="margin-top:8px;text-align:center">${IS_DEMO ? 'Demo mode — simulated instant consecration.' : 'Secure payment via Stripe. Immutable cryptographic ledger recording.'}</p>
    ...
    // --- State & Handlers ---
    const wiz = document.getElementById('createMemorialModal') || document.getElementById('griefWizard');
    const updatePhotoPreview = () => {
      const sp = document.getElementById('gwSpecies')?.value || 'dog';
      const prev = document.getElementById('gwPhotoPreview');
      const status = document.getElementById('gwPhotoStatus');
      const removeBtn = document.getElementById('gwRemovePhotoBtn');
      if (currentPhotoDataUrl) {
        if (prev) prev.innerHTML = `<img src="${currentPhotoDataUrl}" alt="Portrait preview">`;
        if (status) status.textContent = 'Portrait ready for consecration';
        if (removeBtn) removeBtn.classList.remove('hidden');
      } else {
        if (prev) prev.innerHTML = speciesIcon(sp, { size: 46 });
        if (status) status.textContent = 'No photo uploaded yet (optional)';
    ...
    const updateTierPricing = () => {
      ...
      const distKey = document.getElementById('gwDistrict')?.value || 'meadows';
      const d = DISTRICTS[distKey];
      const card = document.getElementById('gwTierPricingCard');
      if (!card) return;
      if (pt === 'sanctuary') {
        const price = d?.base || 249;
        card.innerHTML = `
          <div class="gw-tier-header">
            <span class="gw-tier-title">${icon('sparkle', { size: 14 })} 3D Sanctuary Plot · ${esc(d?.name || 'Memorial Meadows')}</span>
            <span class="gw-tier-price">$${price}</span>
          </div>
          <ul class="gw-tier-perks">
            ...
            <li>${Math.round(SPLITS.plot.charity * 100)}% charitable pass-through tithe to verified animal rescues</li>
            <li>Public SHA-256 cryptographic ledger registry certificate</li>
          </ul>`;
      } else {
        card.innerHTML = `
          <div class="gw-tier-header">
            <span class="gw-tier-title">${icon('globe', { size: 14 })} Global Sacred Footprint Pin on Earth</span>
    ...
    const updateSplitCalc = () => {
      const pt = document.getElementById('gwPlaceType')?.value || 'sanctuary';
      const distKey = document.getElementById('gwDistrict')?.value || 'meadows';
      const d = DISTRICTS[distKey];
      const price = pt === 'sanctuary' ? (d?.base || 249) : 15;
      const priceCents = toCents(price);
      const s = split('plot', priceCents);
      const chId = document.getElementById('gwCharity')?.value || CHARITIES[0]?.id;
      const ch = charityById(chId) || CHARITIES[0];
      const impact = calculateImpact(s.charity);

      const splitEl = document.getElementById('gwSplitCalc');
      if (splitEl) {
        splitEl.innerHTML = `
          <div class="gw-split-row"><span>Plot Consecration Tier</span><b>${fmt(s.gross)}</b></div>
          <div class="gw-split-row is-fee"><span>Direct processing (Stripe rate)</span><b>−${fmt(s.processor)}</b></div>
          <div class="gw-split-row is-fee"><span>Sanctuary 3D infrastructure (ops)</span><b>−${fmt(s.ops)}</b></div>
          <div class="gw-split-row is-total"><span>Delivered directly to ${esc(ch.name)}</span><b>${fmt(s.charity)}</b></div>`;
      }
      const calloutEl = document.getElementById('gwImpactCallout');
      if (calloutEl) {
        calloutEl.innerHTML = `✦ <b>Real-World Impact:</b> This memorial provides <b>${impact.mealsProvided} warm meals</b> and medical support for rescued shelter animals at <b>${esc(ch.name)}</b>.`;
      }
    };

    const goStep = (n) => {
      if (typeof n !== 'number' || isNaN(n) || n < 1 || n > 5) return;
      // Step 1 Validation
      if (n > 1) {
        const nameVal = document.getElementById('gwName')?.value?.trim();
        if (!nameVal) {
          this.toast('Please enter your companion’s name to continue.', 'warning');
      ...
      const mb = wiz.closest('.modal-body');
      if (mb) mb.scrollTo({ top: 0, behavior: 'smooth' });

      if (n === 2) updatePhotoPreview();
      if (n === 4) {
        updateTierPricing();
      ...
      if (n === 5) {
        updateSplitCalc();
        this._gwUpdateSummary(currentPhotoDataUrl);
      }
    };
    ...
    const photoInput = document.getElementById('gwPhoto');
    const chooseBtn = document.getElementById('gwChoosePhotoBtn');
    const removeBtn = document.getElementById('gwRemovePhotoBtn');
    ...
    photoInput?.addEventListener('change', async () => {
      ...
      if (f) {
        if (f.size > 10 * 1024 * 1024) {
          this.toast('Photo exceeds 10MB limit. Please choose a smaller photo.', 'warning');
          photoInput.value = '';
          return;
        }
        const dataUrl = await readOneFile(f, 420);
        if (dataUrl) {
          currentPhotoDataUrl = dataUrl;
          updatePhotoPreview();
          this.toast('Portrait attached successfully!', 'photo');
        } else {
          this.toast('Could not process this image format. Please try a standard JPG or PNG.', 'warning');
          photoInput.value = '';
    ...
    // Step 3: Epitaph char counter & prompts
    const epitaphInput = document.getElementById('gwEpitaph');
    const countEl = document.getElementById('gwEpitaphCount');
    const updateCount = () => { if (countEl && epitaphInput) countEl.textContent = String(epitaphInput.value.length); };
    epitaphInput?.addEventListener('input', updateCount);
    updateCount();

    wiz.querySelectorAll('.gw-prompt').forEach(p => {
      p.onclick = () => {
        if (epitaphInput) {
          epitaphInput.value = p.dataset.txt;
          updateCount();
        }
      };
    });

    // Step 4: Place type toggle & District grid
    ...
    // Create Button
    document.getElementById('gwCreateBtn').onclick = () => this._gwCreate(null, currentPhotoDataUrl);
  },

  _gwUpdateSummary(photoDataUrl = null) {
    ...
    const earthLoc = document.getElementById('gwEarthLoc')?.value?.trim() || 'Sacred Earth Spot';
    const charity = charityName(document.getElementById('gwCharity')?.value) || 'Animal Rescue Partner';
    const birth = document.getElementById('gwBirthYear')?.value?.trim() || '';
    const pass = document.getElementById('gwPassYear')?.value?.trim() || '';
    const years = [birth, pass].filter(Boolean).join(' – ') || 'Forever';
      ...
      el.innerHTML = `
        <div class="gw-summary-card">
          <div class="gw-summary-icon">
            ${photoDataUrl ? `<img src="${photoDataUrl}" alt="Companion Photo" style="width:100%;height:100%;object-fit:cover;border-radius:50%">` : speciesIcon(sp, { size: 28 })}
          </div>
          <div class="gw-summary-body">
            <b>${esc(name)} (${years})</b>
            <span>${pt === 'sanctuary' ? (d?.name || 'Sanctuary 3D Valley') : esc(earthLoc)} · ${icon('heart', { size: 12 })} Beneficiary: ${esc(charity)}</span>
          </div>
        </div>`;
    }
  },

  async _gwCreate(cached = null, activePhoto = null) {
    if (this._isCreating) return;
    this._isCreating = true;
    try {
      const birth = cached?.birthYear || document.getElementById('gwBirthYear')?.value?.trim() || '';
      const pass = cached?.passYear || document.getElementById('gwPassYear')?.value?.trim() || '';
      const computedYears = cached?.years || (birth && pass ? `${birth} – ${pass}` : birth || pass || document.getElementById('gwYears')?.value?.trim() || String(new Date().getFullYear()));
      const rawPhoto = cached?.photo || activePhoto || await readPhoto(document.getElementById('gwPhoto'));
      ...
      if (!formData.name) {
        return this.toast('Please enter your companion’s name.', 'warning');
      }
      ...
      if (!IS_DEMO && !State.hasMembership()) {
        this._cachedWizardData = formData;
      ...
      if (btn) { btn.textContent = 'Consecrating memorial…'; btn.disabled = true; }
      ...
      if (pt === 'globe') {
        ...
        try {
          ...
          if (r.ok) {
            ...
            this.toast(r.error || 'Payment could not be processed.', 'warning');
            if (btn) { btn.innerHTML = `${icon('crest')} Consecrate Their Memorial`; btn.disabled = false; }
          }
        } catch (e) {
      ...
      if (!availPlots.length) {
        this.toast(`No available plots found in ${DISTRICTS[district]?.name || 'the sanctuary'}.`);
        if (btn) { btn.innerHTML = `${icon('crest')} Consecrate Their Memorial`; btn.disabled = false; }
        return;
      }
  ...
  profileModal(afterSave) {
    ...
    this.modal(`
      ...
      <div style="display:flex;gap:14px;align-items:center;margin:10px 0">
        ${p.avatar ? `<img src="${p.avatar}" alt="Pet Avatar" class="pet-photo" style="width:64px;height:64px">` : `<div class="crest-lg">${icon('crest', { size: 44 })}</div>`}
    ...
    $('#soSave').onclick = async () => {
      ...
      if (newName) { Auth.user.name = newName; try { localStorage.setItem('ev_user', JSON.stringify(Auth.user)); } catch {} }
      ...
      await State.save(Auth.user, true);
      if (newName) Auth._emit();
      this.closeModal();
  ...
  myBridgeModal() {
    ...
    this.modal(`
      <div style="display:flex;gap:16px;align-items:center">
        ${p.avatar ? `<img src="${p.avatar}" alt="Pet Avatar" class="pet-photo" style="width:72px;height:72px">` : `<div class="crest-lg">${icon('crest', { size: 52 })}</div>`}
        <div>
  ...
  _initEarthUI() {
    ...
    $('#tourBtn')?.addEventListener('click', () => this.show3D('tour'));
  ...
  async mountEarth() {
    if (!this.earth) {
      const { EarthView } = await import('./earth.js?v=26');
  ...
  async shelterModal(ch) {
    const campaigns = (await Campaigns.load()).filter(c => c.charityId === ch.id);
    this.modal(`
    ...
    box.querySelectorAll('[data-cmp]').forEach(el => {
      el.onclick = async () => {
        const c = await Campaigns.get(el.dataset.cmp);
  ...
  adminHudModal() {
    ...
    box.querySelector('#admBlessing').onclick = () => {
      if (this.world) { this.world.mood = 'blessing'; this.world.lighting.applyAmbience(); }
  ...
  partnerModal() {
    ...
    box.querySelector('#ptSubmit').onclick = async () => {
      ...
      try {
        const cfg = await import('./config.js?v=26');
        if (cfg.HAS_API) {
  ...
  soundModal() {
    ...
    const curSolf = Soundscape.solfeggio;
    const curBin = Soundscape.binaural;
    const curVol = Math.round(Soundscape.volume * 100);

    const solfHtml = Object.entries(Soundscape.SOLFEGGIO).map(([key, s]) => `
      <button class="solf-pill ${curSolf === key ? 'is-active' : ''}" data-solf="${key}" title="${s.desc}">
        <b>${s.freq}Hz</b>
        <span>${s.name.replace(/^\d+Hz\s*/, '')}</span>
      </button>
    `).join('');

    const binHtml = Object.entries(Soundscape.BINAURAL).map(([key, b]) => `
      <button class="binaural-chip ${curBin === key ? 'is-active' : ''}" data-bin="${key}" title="${b.desc}">
        ${b.name}
      </button>
    `).join('');
    ...
    this.modal(`
      ...
      <div class="sound-modes">
        ...
        <button class="sound-mode-card ${curMode === 'binaural' ? 'is-active' : ''}" data-sm="binaural">
          <div class="sm-icon">${icon('sparkle')}</div>
          <b>Binaural Solace</b>
      ...
      <div class="sound-section-title">
        ...
        <span style="font-size:10px;text-transform:none;letter-spacing:normal;color:var(--text-muted)">Active: <b>${Soundscape.SOLFEGGIO[curSolf]?.name || '432Hz'}</b></span>
      </div>
      ...
      <div class="sound-fx-buttons">
        <button class="btn btn-gold btn-sm btn-block" id="ringBowlBtn">${icon('sparkle')} 528Hz Miracle Bell</button>
        <button class="btn btn-outline btn-sm btn-block" id="gong432Btn">${icon('sparkle')} 432Hz Earth Gong</button>
        <button class="btn btn-outline btn-sm btn-block" id="crown963Btn">${icon('sparkle')} 963Hz Crown Tone</button>
      </div>
    ...
    // Solfeggio Buttons
    box.querySelectorAll('[data-solf]').forEach(btn => {
      btn.onclick = () => {
        const freqKey = btn.dataset.solf;
        Soundscape.setSolfeggio(freqKey);
        box.querySelectorAll('[data-solf]').forEach(b => b.classList.remove('is-active'));
        btn.classList.add('is-active');
        const s = Soundscape.SOLFEGGIO[freqKey];
        Soundscape.playSolfeggioBell(s.freq);
        this.toast(`Tuned to ${s.name} (${s.label})`, 3500, 'sparkle');
      };
    });

    // Binaural Preset Buttons
    box.querySelectorAll('[data-bin]').forEach(btn => {
      btn.onclick = () => {
        const binKey = btn.dataset.bin;
        Soundscape.setBinaural(binKey);
        box.querySelectorAll('[data-bin]').forEach(b => b.classList.remove('is-active'));
        btn.classList.add('is-active');
        const b = Soundscape.BINAURAL[binKey];
        this.toast(`Binaural: ${b.name} (${b.desc})`, 3500, 'sparkle');
      };
    });
    ...
    box.querySelector('#gong432Btn').onclick = () => {
      Soundscape.playBowlGong(216, 0.22);
      const rect = document.body.getBoundingClientRect();
      Motion.spark(rect.width / 2, rect.height / 2, 35);
    };

    box.querySelector('#crown963Btn').onclick = () => {
      Soundscape.playSolfeggioBell(963);
      const rect = document.body.getBoundingClientRect();
      Motion.spark(rect.width / 2, rect.height / 2, 35);
    };
            ...
            <div class="keepsake-mockup" id="ksMockup">
              <div class="ks-mockup-frame ks-mockup--${selectedItem.category.toLowerCase()}">
                <div class="ks-photo-slot">
                  ${selectedPhoto ? `<img src="${selectedPhoto}" alt="Keepsake Photo" class="ks-rendered-img">` : `
    ...
    $('#emPay').onclick = async () => {
      ...
      try {
        ...
        if (r.ok) {
          ...
          const rawEpitaph = cleanText($('#emEpitaph').value.trim()) || 'Forever loved.';
          
          btn.textContent = 'Generative Processing...';
          const [videoUrl, omniData] = await Promise.all([
            photo ? generateVeoVideo(photo, 'Cinematic memorial loop') : Promise.resolve(null),
            generateOmniMemorial(photo, rawEpitaph)
          ]);
          ...
          const mem = {
            ...
            epitaph: omniData.epitaph || rawEpitaph,
            photo,
            videoUrl,
            audioUrl: omniData.audioUrl,
            mood: omniData.mood,
  ...
  toggleFeed() {
    ...
    for(let i=0; i<12; i++) {
      ...
      if(type < 0.35 && occupiedPlots.length > 0) {
        ...
        items.push({
          icon: icon('heart'),
          html: `<b>${escapeHtml(petName)}'s Rainbow Fund</b> — community support active`,
          time: timeLabel
        });
      }
    }

    // Real metrics from Firestore state (fallback to '—' in demo mode)
    const isDemo = typeof State === 'undefined' || !State.data || State.data._demo;
    const totalMemorials = isDemo ? '—' : (State.data.stats?.totalMemorials ?? 0);
    const giftsToday = isDemo ? '—' : (State.data.stats?.giftsToday ?? 0);
    const raisedMonth = isDemo ? '—' : (State.data.stats?.raisedMonth ?? 0);
      ...
      if (submitBtn) {
        submitBtn.onclick = async () => {
          ...
          try {
            ...
            if (this.toast) this.toast('Soundscape playback failed.', 'warning');
          }
          ...
          try {
            ...
            if (this.toast) this.toast('Failed to record offering.', 'warning');
          }
