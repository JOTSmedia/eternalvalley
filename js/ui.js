...
// ============================================================
import { DISTRICTS, SIZE_LABELS, PET_NAMES, SPECIES, EPITAPHS } from './terrain.js?v=9';
import { MEMBERSHIPS, PLOT_ITEMS, GIFTS, EARTH_PLOT, PHYSICAL_KEEPSAKES, SLOTS, HEADSTONE_STYLES, ITEM_DECOR, GIFT_DECOR, CHARITIES, GIFT_CHARITY_SHARE, charityName, fmtPrice } from './catalog.js?v=9';
import { CharityUI } from './charityui.js?v=9';
import { Tour } from './tour.js?v=9';
import { SPLITS, Campaigns, Ledger, fmt, charityById, toCents, split } from './charity.js?v=9';
import { Auth } from './auth.js?v=9';
import { State } from './state.js?v=9';
import { checkout } from './checkout.js?v=9';
import { IS_DEMO, HAS_MAPS3D, IS_ADMIN } from './config.js?v=9';
import { RBV, allMemorials, allActivity, timeAgo } from './social.js?v=9';
import { Motion } from './motion.js?v=9';
import { icon, speciesIcon, speciesKey, rainbowMark, SPECIES_LABELS } from './icons.js?v=9';
import { thumbImg, canRender as canRenderThumb, photoFor } from './thumbs.js?v=9';
import { Soundscape } from './soundscape.js?v=9';
import { checkAnniversaries, generateICS, ANNIVERSARY_GIFTS } from './anniversary.js?v=9';
...
export const UI = {
    ...
    if (!this.globe) {
      if (this._globePromise) {
        ...
        this._globePromise = (async () => {
          const { Globe } = await import('./globe.js?v=9');
  ...
  async show3D(mode = 'tour', isEntrance = false) {
    ...
    // Cinematic Film Tour option
    if (mode === 'film') {
      const landing = document.getElementById('veoLanding');
      const video = document.getElementById('veoVideo');
      if (landing && video) {
        // Dynamically request video from Veo AI
        import('./api.js').then(api => {
          api.generateVeoVideo(null, 'Cinematic drone tour').then(url => {
            video.src = url;
            video.play().catch(e => { console.warn('Autoplay prevented:', e); this.show3D('tour'); });
          });
        });

        this._veoActive = true;
        ...
        // Pre-warm 3D world in background so transitioning from video to 3D has zero lag
        if (!this.world) this.ensureWorld().catch(() => {});

        const v3d = document.getElementById('view3d'); if (v3d) { v3d.classList.add('hidden'); v3d.classList.remove('is-active'); }
        this._setupVeoInteractiveHUD(landing, video);
        return;
      }
    }

    // When in 3D modes (tour, orbit, walk), hide video overlay and pause video
    const landing = document.getElementById('veoLanding');
    if (landing && landing.style.display !== 'none') {
      landing.style.display = 'none';
      const video = document.getElementById('veoVideo');
      if (video) video.pause(); pin.style.transform = "translate(-50%, -100%) scale(1.0)"; video.src = ""; video.load();
  ...
  _setupVeoInteractiveHUD(landing, video) {
    ...
    const dismissVideo = (flyKey = null, targetMode = 'tour') => {
      ...
      setTimeout(() => {
        ...
        landing.style.opacity = '1';
        landing.style.transition = '';
        video.pause(); pin.style.transform = "translate(-50%, -100%) scale(1.0)"; video.src = ""; video.load();
    ...
    if (vpcFly3dBtn) {
      vpcFly3dBtn.onclick = (e) => {
        e.stopPropagation();
        dismissVideo(plot ? plot.flyKey : null, 'orbit');
      };
    }
  ...
  async show2D() {
    if (!this.map) {
      if (window.UI?.map) this.map = window.UI.map;
        ...
        if (!this.map) {
          try {
            const { Map2D } = await import('./map2d.js?v=9');
  ...
  async mountEarth() {
    if (!this.earth) {
      const { EarthView } = await import('./earth.js?v=9');
  ...
  partnerModal() {
    ...
    box.querySelector('#ptSubmit').onclick = async () => {
      ...
      try {
        const cfg = await import('./config.js?v=9');
        if (cfg.HAS_API) {
