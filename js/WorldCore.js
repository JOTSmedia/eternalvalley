...
export class World3DCore {
  ...
  _resize() {
    ...
    if (this.lighting && this.lighting.composer) {
       ...
       if (this.lighting.bloomPass) this.lighting.bloomPass.setSize(Math.ceil((w * targetDpr) / 2), Math.ceil((h * targetDpr) / 2));
    }
    ...
    if (this.lighting._cinematicPass?.uniforms?.uResolution) {
      this.lighting._cinematicPass.uniforms.uResolution.value.set(w * targetDpr, h * targetDpr);
    }
  }
...
//     matMossBoulder.color.setHex(0x4a5d4e);
    ...
    this._onPointerUp = e => {
      ...
      if (this.tourController.tourMode) {
        if (hits.length) {
          ...
          if (plot) {
            // Pause drone immediately so user can examine and reserve the plot
            this._tourPaused = true;
            this.tourController._updateTourHUD(this.tourController._tourTime, true);
        ...
        // Click on background canvas toggles pause/play in drone tour
        this.tourController.toggleTourPlayPause();
        return;
      }
  ...
  flyToDistrict(key, plot) {
    if (this.tourController) this.tourController.flyToDistrict(key, plot);
  }

  flyToCathedral(view = 'exterior') {
    if (this.tourController) {
      if (typeof this.tourController.flyToCathedral === 'function') {
        this.tourController.flyToCathedral(view);
      } else {
        this.tourController.flyToDistrict(view === 'interior' ? 'cathedral_interior' : 'cathedral');
      }
    }
  }
  ...
  _animate() {
    ...
    try {
      ...
      if (now - this._lastFpsHudUpdate >= 200 && this._fpsCount >= 5) {
        ...
        if (!this._drsInitialized) {
          ...
          this._drsPixelRatio = Math.min(window.devicePixelRatio || 1, isMob ? 1.5 : 2.0);
      ...
      this._accumTime = (this._accumTime || 0) + dt; const t = this._accumTime;
    ...
    this._updateUnderwaterAtmosphere(dt);
      ...
      try { if (typeof videoManager !== "undefined" && videoManager) videoManager.update(); } catch(ve) { console.warn("videoManager error:", ve); }
  ...
  _updateUnderwaterAtmosphere(dt) {
    ...
    // 1. Highland Glacial Tarn Water Basin (y=182.0m, deep basin floor y=158m, z=-460..-635, |x| < 62)
    ...
    // FIX: Updated Z coordinate from -360 to -550 to match actual plunge pool location
    ...
    // FIX: Added universal fallback `camY < WORLD.waterLevel` to ensure fog triggers whenever underwater
    ...
    // 4. Southern Ocean Coral Reef & Abyss
    ...
    this._colSunlitAqua = this._colSunlitAqua || new THREE.Color(0x60e0ff);
    this._colDeepAbyssal = this._colDeepAbyssal || new THREE.Color(0x1878a8);
    ...
    if (isUnderOcean) {
      ...
      const extinction = Math.exp(-depth * 0.015);
      ...
      targetDensity = 0.0003 + (1.0 - extinction) * 0.0002;
      targetNear = 15.0;
    ...
    // Volumetric bubble & god ray opacity
    if (this._bubbleMat && this._bubbleMat.uniforms && this._bubbleMat.uniforms.uOpacity) {
      this._bubbleMat.uniforms.uOpacity.value = this._underwaterBlend;
    }
    if (this._godRaysMat && this._godRaysMat.uniforms && this._godRaysMat.uniforms.uOpacity) {
      this._godRaysMat.uniforms.uOpacity.value = this._underwaterBlend;
    }
  }
