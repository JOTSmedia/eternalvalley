...
//     darkBronze.metalness = 0.88;
  ...
  _createPhysicalWaterMaterial(normals, type = 'lake') {
    ...
    if (window.videoManager) {
      waterTex = window.videoManager.createVideoTexture('water_' + type, 'https://storage.googleapis.com/gtv-videos-bucket/sample/ForBiggerBlazes.mp4', false);
    }
    
    const mat = new THREE.MeshBasicMaterial({
      map: waterTex || fallbackTex,
      color: 0x4488aa,
      transparent: true,
      opacity: 0.85
    });
    
    // Hack to attach the registerMesh callback after instantiation
    mat.onBeforeCompile = (shader) => {
      // Intentionally left blank, just for backwards compatibility if something tries to modify it
    };
