...
//     if (palmBark && palmBark.color) palmBark.color.setHex(0xa88e6e);
    ...
    // Monumental Imperial Polished Black Granite & 24K Gold Pedestal for Pure-Bred Siberian Husky KAYA
    const pedestalGroup = new THREE.Group();
    pedestalGroup.position.set(0, 2.8, 0);

    const graniteMat = new THREE.MeshPhysicalMaterial({
      color: 0x121518,
      roughness: 0.16,
      metalness: 0.25,
      clearcoat: 0.9,
    ...
    const goldTrimMat = Surfaces.gold(1.0);

    // Tier 1 Base Plinth
    const plinthBase = new THREE.Mesh(new THREE.CylinderGeometry(4.4, 4.8, 0.7, 32), graniteMat);
    plinthBase.position.y = 0.35;
    plinthBase.castShadow = true;
    pedestalGroup.add(plinthBase);

    // Gilded Transition Molding
    const plinthTrim1 = new THREE.Mesh(new THREE.TorusGeometry(4.42, 0.12, 12, 32), goldTrimMat);
    plinthTrim1.rotation.x = Math.PI / 2;
    plinthTrim1.position.y = 0.70;
    pedestalGroup.add(plinthTrim1);

    // Tier 2 Middle Dado Column
    const plinthMid = new THREE.Mesh(new THREE.CylinderGeometry(3.8, 4.1, 0.95, 32), graniteMat);
    plinthMid.position.y = 1.18;
    plinthMid.castShadow = true;
    pedestalGroup.add(plinthMid);

    // Gilded Upper Cornice
    const plinthCap = new THREE.Mesh(new THREE.CylinderGeometry(4.1, 3.7, 0.35, 32), goldTrimMat);
    plinthCap.position.y = 1.78;
    plinthCap.castShadow = true;
    pedestalGroup.add(plinthCap);

    // Inscribed Bronze/Gold Tablet on the front facing North toward entrance (-Z)
    const tabletGeo = new THREE.BoxGeometry(3.6, 0.95, 0.14);
    ...
    if (tctx) {
      tctx.fillStyle = '#0f1216';
      tctx.fillRect(0, 0, 1024, 256);
      tctx.strokeStyle = '#d4af37';
      tctx.lineWidth = 8;
      tctx.strokeRect(14, 14, 996, 228);
      tctx.fillStyle = '#fef08a';
      tctx.font = 'bold 54px "Cinzel", "Times New Roman", Georgia, serif';
      tctx.textAlign = 'center';
      tctx.textBaseline = 'middle';
      tctx.fillText('K A Y A', 512, 68);
      tctx.font = 'italic 30px "Cinzel", "Times New Roman", serif';
      tctx.fillStyle = '#e2e8f0';
      tctx.fillText('Pure-Bred Siberian Husky', 512, 132);
      tctx.font = '22px "Cinzel", "Times New Roman", serif';
      tctx.fillStyle = '#cbd5e1';
      tctx.fillText('Beloved Guardian Spirit of Eternal Valley', 512, 188);
    }
    ...
    tablet.position.set(0, 1.18, -3.85);
    pedestalGroup.add(tablet);
    pavGroup.add(pedestalGroup);

    // Front Inscribed Dedication Plaque (North pavilion entrance facing the valley and bridge approach)
    const plaqueGeo = new THREE.BoxGeometry(7.2, 1.2, 0.35);
    const plaqueCanvas = document.createElement('canvas');
    plaqueCanvas.width = 1024;
    plaqueCanvas.height = 160;
    const pctx = plaqueCanvas.getContext('2d');
    if (pctx) {
      pctx.fillStyle = '#0f1216';
      pctx.fillRect(0, 0, 1024, 160);
      pctx.strokeStyle = '#d4af37';
      pctx.lineWidth = 6;
      pctx.strokeRect(10, 10, 1004, 140);
      pctx.fillStyle = '#fef08a';
      pctx.font = 'bold 36px "Cinzel", "Times New Roman", serif';
      pctx.textAlign = 'center';
      pctx.textBaseline = 'middle';
      pctx.fillText('KAYA ISLAND — SANCTUARY OF THE ARCTIC GUARDIAN', 512, 60);
      pctx.font = 'italic 22px "Cinzel", "Times New Roman", serif';
      pctx.fillStyle = '#e2e8f0';
      pctx.fillText('Dedicated to Kaya · Pure-Bred Siberian Husky', 512, 108);
    }
    ...
    // 3D Pure-Bred Siberian Husky KAYA Monument (Standing proudly atop pedestal facing North out pavilion entrance)
    const huskyKaya = this._buildHuskyMesh();
    huskyKaya.scale.setScalar(2.0);
    huskyKaya.position.set(0, 4.8, 0);
    huskyKaya.rotation.y = Math.PI; // Face North toward incoming drone approach at z = 2040
    huskyKaya.castShadow = true;
    huskyKaya.userData = {
      isKayaMonument: true,
      petName: 'Kaya',
      species: 'Pure-Bred Siberian Husky',
      title: 'Kaya — Pure-Bred Siberian Husky Guardian',
      bio: 'Beloved Pure-Bred Siberian Husky, eternal watchful guardian of the Rainbow Bridge and protector of all companion spirits resting in Eternal Valley.'
    };
    ...
    for (let p = 0; p < 380; p++) {
      ...
      if (px >= 0 && px <= 40 && pz >= 2020 && pz <= 2095) continue; // Clear hero drone sightline corridor to Kaya Starlight Pavilion
  ...
  _buildUniversalStructure(cx, cz, idPrefix, plotName) {
    ...
    // 2. Main Keep (Open Interior Nave with Sandstone Flanking Walls, Altar Apse, and Archway Portal)
    const wallThick = 4.0;
    const keepW = 52, keepH = 46, keepL = 74;
    const halfW = keepW / 2, halfL = keepL / 2;

    // Left & Right Flank Walls
    const wallLeft = new THREE.BoxGeometry(wallThick, keepH, keepL);
    wallLeft.translate(-halfW + wallThick / 2, 27 + keepH / 2, 0);
    const wallRight = new THREE.BoxGeometry(wallThick, keepH, keepL);
    wallRight.translate(halfW - wallThick / 2, 27 + keepH / 2, 0);

    // North Altar Wall (Rear Apse)
    const wallRear = new THREE.BoxGeometry(keepW, keepH, wallThick);
    wallRear.translate(0, 27 + keepH / 2, -halfL + wallThick / 2);

    // South Portal Facade (Front Entrance with grand open archway into Asamkirche nave)
    const portalPillarW = 16;
    const portalLeft = new THREE.BoxGeometry(portalPillarW, keepH, wallThick);
    portalLeft.translate(-halfW + portalPillarW / 2, 27 + keepH / 2, halfL - wallThick / 2);
    const portalRight = new THREE.BoxGeometry(portalPillarW, keepH, wallThick);
    portalRight.translate(halfW - portalPillarW / 2, 27 + keepH / 2, halfL - wallThick / 2);
    const portalLintelH = 16;
    const portalLintel = new THREE.BoxGeometry(keepW - portalPillarW * 2, portalLintelH, wallThick);
    portalLintel.translate(0, 27 + keepH - portalLintelH / 2, halfL - wallThick / 2);

    sandstoneGeos.push(wallLeft, wallRight, wallRear, portalLeft, portalRight, portalLintel);

    // Main Keep Roof Crest
    const keepRoofGeo = new THREE.ConeGeometry(36, 22, 4);
    keepRoofGeo.rotateY(Math.PI / 4); // align to box
    keepRoofGeo.translate(0, 27 + keepH + 11, 0);
    slateGeos.push(keepRoofGeo);

    // 3. Hohenzollern Fortress Turrets and Towers (Framing the Citadel & Bastion)
    const towerPositions = [
        {x: -26, z: 35, r: 7.5, h: 65, rh: 28},
        {x: 26, z: 35, r: 7.5, h: 65, rh: 28},
        {x: -26, z: -35, r: 7.5, h: 65, rh: 28},
        {x: 26, z: -35, r: 7.5, h: 65, rh: 28},
        {x: -42, z: 46, r: 5.5, h: 45, rh: 20},
        {x: 42, z: 46, r: 5.5, h: 45, rh: 20},
        {x: -42, z: -46, r: 5.5, h: 45, rh: 20},
        {x: 42, z: -46, r: 5.5, h: 45, rh: 20},
        {x: -20, z: 48, r: 6.5, h: 56, rh: 25}, // West Barbican Gatehouse Tower
        {x: 20, z: 48, r: 6.5, h: 56, rh: 25},  // East Barbican Gatehouse Tower
        {x: 0, z: -42, r: 8.5, h: 75, rh: 36}   // North Apse Citadel Tower (Behind High Altar)
    ];

    towerPositions.forEach(pos => {
        const tower = new THREE.CylinderGeometry(pos.r * 0.9, pos.r, pos.h, 16);
        tower.translate(pos.x, 27 + pos.h/2, pos.z);
        sandstoneGeos.push(tower);

        const roof = new THREE.ConeGeometry(pos.r * 1.25, pos.rh, 16);
        roof.translate(pos.x, 27 + pos.h + pos.rh/2, pos.z);
        slateGeos.push(roof);
    });

    // Soaring Central Hohenzollern Keep Spire (Sits majestically on the roof above y=73)
    const centralSpire = new THREE.CylinderGeometry(6.5, 8.5, 42, 16);
    centralSpire.translate(0, 27 + keepH + 21, -5);
    sandstoneGeos.push(centralSpire);

    const centralSpireRoof = new THREE.ConeGeometry(11, 46, 16);
    centralSpireRoof.translate(0, 27 + keepH + 42 + 23, -5);
    slateGeos.push(centralSpireRoof);
    ...
    // --- Interior Geometry (Asamkirche Late Rococo Sanctuary) ---
    const marbleGeos = [];
    const goldGeos = [];

    // Polished Asam Red Marble Floor
    const floorGeo = new THREE.BoxGeometry(keepW - wallThick * 2, 2, keepL - wallThick * 2);
    floorGeo.translate(0, 27 + 1, 0);
    marbleGeos.push(floorGeo);

    // Asamkirche High Altar (North Apse)
    const altarBase = new THREE.BoxGeometry(18, 5, 6);
    altarBase.translate(0, 27 + 3.5, -halfL + 8);
    marbleGeos.push(altarBase);

    const altarpiece = new THREE.BoxGeometry(14, 18, 2);
    altarpiece.translate(0, 27 + 15, -halfL + 6.5);
    goldGeos.push(altarpiece);

    // Solomonic (twisted) columns simulation along the nave
    for (let z = -22; z <= 18; z += 10) {
        for (let x of [-14, 14]) {
            // Twisted column effect
            for(let y = 29; y < 65; y += 2) {
    ...
    // Gilded Rococo Balconies flanking the nave
    const bLeft = new THREE.BoxGeometry(3, 2, 48); bLeft.translate(-14.5, 45, 0);
    const bRight = new THREE.BoxGeometry(3, 2, 48); bRight.translate(14.5, 45, 0);
    ...
    // Vaulted Fresco Ceiling (DoubleSide so illuminated fresco is visible from nave below)
    matFresco.side = THREE.DoubleSide;
    const ceilingGeo = new THREE.CylinderGeometry(18, 18, 54, 32, 1, true, 0, Math.PI);
    ...
    // Chiaroscuro Lighting (Dramatic luminous clerestory rays illuminating golden altar)
    const ambientLight = new THREE.AmbientLight(0x3a2220, 1.2); 
    group.add(ambientLight);

    const sunBeam = new THREE.SpotLight(0xffeedd, 35);
    sunBeam.position.set(0, 88, -10);
    sunBeam.target.position.set(0, 32, -26);
    sunBeam.angle = Math.PI / 4;
    sunBeam.penumbra = 0.6;
    ...
    // --- Purchasable Interior Balcony & Roof Plots ---
    if (this.world && Array.isArray(this.world.plots)) {
      const balconyPlots = [
        { lx: -12, lz: -18, rot: Math.PI / 2, name: `${plotName} Fresco Balcony West 1` },
        { lx: -12, lz: -6,  rot: Math.PI / 2, name: `${plotName} Fresco Balcony West 2` },
        { lx: -12, lz: 6,   rot: Math.PI / 2, name: `${plotName} Fresco Balcony West 3` },
        { lx: -12, lz: 18,  rot: Math.PI / 2, name: `${plotName} Fresco Balcony West 4` },
        { lx: 12,  lz: -18, rot: -Math.PI / 2, name: `${plotName} Fresco Balcony East 1` },
        { lx: 12,  lz: -6,  rot: -Math.PI / 2, name: `${plotName} Fresco Balcony East 2` },
        { lx: 12,  lz: 6,   rot: -Math.PI / 2, name: `${plotName} Fresco Balcony East 3` },
        { lx: 12,  lz: 18,  rot: -Math.PI / 2, name: `${plotName} Fresco Balcony East 4` },
      ];
      balconyPlots.forEach((bp, idx) => {
        const id = `${idPrefix}_ROOF_${idx + 1}`;
        if (!this.world.plots.some(p => p.id === id)) {
          this.world.plots.push({
            id,
            x: cx + bp.lx,
            z: cz + bp.lz,
            rot: bp.rot,
            type: idPrefix.toLowerCase(),
            name: bp.name,
            size: 'estate',
            tier: 3,
            status: 'available',
...
//     darkBronze.metalness = 0.88;
  ...
  _buildHuskyMesh() {
    ...
    husky.userData = { isKayaMonument: true, petName: 'Kaya', breed: 'Pure-Bred Siberian Husky' };
    ...
    // 1. Athletic Muscular Canine Body (AKC Siberian Husky standard proportions)
    ...
    // 3. Expressive Siberian Husky Head & Face (AKC Breed Standard)
    ...
    // Dark mask cap across forehead / clover marking
    ...
    // Muzzle / Snout: Medium length tapering gradually to black leather nose
    ...
    // Piercing-Blue Almond Husky Eyes with Dark Eyeliner Contours & White Eyebrow Spots
    [-0.17, 0.17].forEach(ex => {
      ...
      // White eyebrow spot characteristic of Siberian Huskies
      const browSpot = new THREE.Mesh(new THREE.SphereGeometry(0.045, 8, 6), snowWhite);
      browSpot.scale.set(1.2, 0.7, 0.5);
      browSpot.position.set(ex * 0.9, 0.22, 0.38);
      headGroup.add(browSpot);
    });

    // Triangular Erect Pointed Ears (Pricked alert forward posture with thick fur)
    ...
    legPositions.forEach(lp => {
      ...
      // Digitigrade Compact Oval Paw with Cushioned Pads & Claws
    ...
    // 6. Glorious Siberian Husky Sickle Plume Tail (Carried in organic proud arch over back - NO segmented spheres)
    const tailGroup = new THREE.Group();
    tailGroup.position.set(0, 1.56, -0.96);

    // Smooth organic Catmull-Rom curve sweeping up and forward over the rump
    const tailCurve = new THREE.CatmullRomCurve3([
      new THREE.Vector3(0, 0.0, 0.0),
      new THREE.Vector3(0, 0.38, -0.16),
      new THREE.Vector3(0, 0.82, -0.25),
      new THREE.Vector3(0, 1.20, -0.12),
      new THREE.Vector3(0, 1.35, 0.14),
      new THREE.Vector3(0, 1.22, 0.40),
      new THREE.Vector3(0, 0.94, 0.54),
      new THREE.Vector3(0, 0.62, 0.58),
    ]);
    ...
    const cGrey = new THREE.Color(0x3a404a);
    const cWhite = new THREE.Color(0xfbfdfd);

    for (let i = 0; i <= slices; i++) {
      const u = i / slices;
      const pt = tailCurve.getPointAt(u);
      const tan = tailCurve.getTangentAt(u).normalize();
      ...
      // Fluffy Siberian Husky fox-brush taper: thickest along mid-arch, tapering to white brush tip
      const radius = (0.24 + 0.12 * Math.sin(u * Math.PI)) * (1.0 - u * 0.42);
      const col = u < 0.52 ? cGrey : cWhite;

      for (let j = 0; j <= radial; j++) {
        const v = j / radial;
        const angle = v * Math.PI * 2;
        const radX = radius * (0.85 + 0.15 * Math.abs(Math.sin(angle)));
        const radY = radius * (1.10 + 0.20 * Math.max(0, Math.cos(angle)));

        const x = pt.x + (normal.x * Math.cos(angle) * radX + binormal.x * Math.sin(angle) * radY);
        const y = pt.y + (normal.y * Math.cos(angle) * radX + binormal.y * Math.sin(angle) * radY);
        const z = pt.z + (normal.z * Math.cos(angle) * radX + binormal.z * Math.sin(angle) * radY);
    ...
    tailGeo.setAttribute('position', new THREE.Float32BufferAttribute(positions, 3));
    tailGeo.setAttribute('uv', new THREE.Float32BufferAttribute(uvs, 2));
    ...
    // Fluffy fur fringe tufts for Arctic double-coat silhouette
    for (let f = 0; f < 8; f++) {
      const fu = 0.25 + (f / 7) * 0.65;
      const fpt = tailCurve.getPointAt(fu);
      const tuftGeo = new THREE.ConeGeometry(0.11, 0.32, 5);
      tuftGeo.scale(1.0, 1.0, 0.4);
      const tuftMat = fu < 0.52 ? wolfGrey : snowWhite;
      const tuft = new THREE.Mesh(tuftGeo, tuftMat);
      tuft.position.copy(fpt);
      tuft.rotation.x = Math.PI * 0.38 + (f % 3) * 0.1;
      tuft.rotation.z = (f % 2 === 0 ? 1 : -1) * 0.22;
  ...
  async _generateProceduralPlots() {
    ...
    // 5. On/in buildings and landmark sanctuaries (Desert Canyon, Mosque, Pagoda, Lake, Waterfall)
    const buildings = [
      { idPrefix: 'DESERT', x: -460, z: 340, r: 90, name: 'Desert Canyon Sanctuary' },
      ...
      { idPrefix: 'WATERFALL', x: 0, z: -460, r: 75, name: 'North Cataract' },
      { idPrefix: 'LAKE', x: (WORLD.lake ? WORLD.lake.x : 380), z: (WORLD.lake ? WORLD.lake.z : -250), r: 80, name: 'Mirror Lake Shallows' }
    ];
    
    buildings.forEach(b => {
      let bCount = 0;
      for (let r = 35; r < b.r; r += 16) {
        ...
        for (let i = 0; i < steps; i++) {
          ...
          if (isClear(px, pz)) {
            addPlot(`${b.idPrefix}_${bCount + 1}`, px, pz, ang, b.idPrefix.toLowerCase(), `${b.name} ${bCount + 1}`, 'estate', 3);
            bCount++;
          }
        }
      }
    ...
    // Dedicated Pueblo Sun-Kiva Interior Sanctuary plots
    const kivaPlots = [
      { id: 'PUEBLO_INT_1', lx: -460, lz: 336, name: 'Pueblo Sun-Kiva Hearthside #1', rot: 0 },
      { id: 'PUEBLO_INT_2', lx: -460, lz: 340, name: 'Pueblo Celestial Skylight Altar #2', rot: Math.PI / 2 },
      { id: 'PUEBLO_INT_3', lx: -456, lz: 342, name: 'Pueblo Canyon Overlook Portal #3', rot: -Math.PI / 2 },
    ];
    kivaPlots.forEach(kp => {
      if (!existingPlotIds.has(kp.id)) {
        this.world.plots.push({
          id: kp.id, x: kp.lx, z: kp.lz, rot: kp.rot, type: 'desert_interior',
          name: kp.name, size: 'estate', tier: 3, status: 'available',
          h: 44.5
        });
        existingPlotIds.add(kp.id);
        newPlotsCount++;
      }
    });
