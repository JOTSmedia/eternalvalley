...
import { OrbitControls } from 'three/addons/controls/OrbitControls.js?v=26';
import { Sky } from 'three/addons/objects/Sky.js?v=26';
import { Water } from 'three/addons/objects/Water.js?v=26';
import { Lensflare, LensflareElement } from 'three/addons/objects/Lensflare.js?v=26';
import { EffectComposer } from 'three/addons/postprocessing/EffectComposer.js?v=26';
import { RenderPass } from 'three/addons/postprocessing/RenderPass.js?v=26';
import { UnrealBloomPass } from 'three/addons/postprocessing/UnrealBloomPass.js?v=26';
import { ShaderPass } from 'three/addons/postprocessing/ShaderPass.js?v=26';
import { OutputPass } from 'three/addons/postprocessing/OutputPass.js?v=26';
import { GTAOPass } from 'three/addons/postprocessing/GTAOPass.js?v=26';
import { mergeGeometries as _rawMergeGeometries } from 'three/addons/utils/BufferGeometryUtils.js?v=26';
import { RGBELoader } from 'three/addons/loaders/RGBELoader.js?v=26';
...
import { WORLD, DISTRICTS, ROADS, RIVER, RIVER_INLET, RIVER_OUTLET, terrainHeight, backgroundMountainElevation, distToRoads, distToRiver, getRiverInfo, riverWaterElevation, fbm, ridgeNoise, mulberry32, SIZE_DIMS } from './terrain.js?v=26';
import { getSeason, SEASON_STYLE, getDayPhase, PHASES, MOODS, fetchWeather } from './ambience.js?v=26';
import { Surfaces, waterNormalTexture, textures, material, createBotanicalFoliageMaterial, clearCache } from './materials.js?v=26';
import { icon, speciesIcon, speciesKey } from './icons.js?v=26';
import { charityName } from './catalog.js?v=26';
import { DRONE_TOUR_LANDMARKS } from './tour.js?v=26';
import { buildGrandBoulevard, buildSecondaryRoad } from './roads.js?v=26';
...
export class WorldTerrain {
  ...
  async _terrain() {
    // Adaptive terrain density: desktop gets high-density subdivision for smooth photoreal curvature, mobile stays performant
    const isMobile = typeof window !== 'undefined' && window.innerWidth <= 768;
    const SEG = isMobile ? 96 : 240;
    const W = 4600, L = 5200;
    ...
    const iceBlueSSS      = new THREE.Color(0x78a8cc);
    // Canyon Desert Palette
    const canyonRed       = new THREE.Color(0xb25d3b);
    const canyonOchre     = new THREE.Color(0xd49b55);
    const canyonShadow    = new THREE.Color(0x6b301f);
    const canyonMesaTop   = new THREE.Color(0xc9a473);
    ...
    for (let i = 0; i < pos.count; i++) {
      ...
      if (z > 1750) {
        ...
        // Grand Canyon / Desert Biome
        const canyonDepth = (250 - h) / 300.0;
        const canyonRock = Math.max(0.0, Math.min(1.0, (0.75 - slope) / 0.25));
        
        c.copy(canyonMesaTop);
        c.lerp(canyonRed, Math.max(0, canyonDepth));
        c.lerp(canyonOchre, Math.max(0, canyonRock));
        c.lerp(canyonShadow, (1.0 - creviceAO));
        
        if (h < 50) {
           c.lerp(sandBeach, Math.max(0.0, Math.min(1.0, (50 - h) / 40.0)));
        }
      } else {
...
#include <logdepthbuf_vertex>
  ...
  _underwaterWorld() {
    ...
    // 3. Submerged Marble Memorial Pedestals with Golden Plaques (Mirror Lake Bed)
    // Modeled after classical submerged memorial altars in veo_underwater_koi_shallows.jpg
    const pedestalCount = 24;
    const pedGeo = new THREE.CylinderGeometry(1.6, 2.0, 3.2, 8);
    const pedMat = new THREE.MeshStandardMaterial({
      color: 0xedebe4,
      roughness: 0.35,
      metalness: 0.05
    });
    const pedMesh = new THREE.InstancedMesh(pedGeo, pedMat, pedestalCount);
    pedMesh.castShadow = true;
    pedMesh.receiveShadow = true;

    // Golden Plaque on top of pedestal
    const plaqueGeo = new THREE.BoxGeometry(1.4, 0.25, 1.4);
    ...
    const plaqueMesh = new THREE.InstancedMesh(plaqueGeo, plaqueMat, pedestalCount);

    const lakeX = (typeof WORLD !== 'undefined' && WORLD.lake) ? WORLD.lake.x : 380;
    const lakeZ = (typeof WORLD !== 'undefined' && WORLD.lake) ? WORLD.lake.z : -250;
    const lakeR = (typeof WORLD !== 'undefined' && WORLD.lake) ? WORLD.lake.r : 130;

    for (let i = 0; i < pedestalCount; i++) {
      const angle = (i / pedestalCount) * Math.PI * 2 + (i % 2) * 0.15;
      const rad = 25 + ((i * 37) % Math.floor(lakeR * 0.65));
      const px = lakeX + Math.cos(angle) * rad;
      const pz = lakeZ + Math.sin(angle) * rad;
      const floorH = typeof terrainHeight === 'function' ? terrainHeight(px, pz) : 2.0;

      dummy.position.set(px, floorH + 1.6, pz);
      dummy.rotation.set(0, angle + Math.PI / 4, 0);
      dummy.scale.set(1.0, 1.0, 1.0);
      dummy.updateMatrix();
      pedMesh.setMatrixAt(i, dummy.matrix);

      dummy.position.set(px, floorH + 3.25, pz);
      dummy.scale.set(1.0, 1.0, 1.0);
      dummy.updateMatrix();
    ...
    uwGroup.add(pedMesh);
    uwGroup.add(plaqueMesh);

    // 4. Submerged Crystalline Memorial Obelisks (Prismatic Cyan & Aquamarine Shrines)
    // Modeled after luminous crystal monoliths in veo_underwater_coral_reef.jpg & koi shallows
    const obeliskCount = 36;
    const obGeo = new THREE.ConeGeometry(1.2, 7.5, 6);
    obGeo.translate(0, 3.75, 0);
    ...
    const obeliskMesh = new THREE.InstancedMesh(obGeo, obMat, obeliskCount);
    ...
    for (let i = 0; i < obeliskCount; i++) {
      ...
      if (i < 12) {
        ...
        const r = 35 + (i * 19) % 55;
        ox = lakeX + Math.cos(a) * r;
        oz = lakeZ + Math.sin(a) * r;
      } else if (i < 26) {
        const a = ((i - 12) / 14) * Math.PI * 2;
        const r = 60 + ((i * 23) % 110);
        ox = 20 + Math.cos(a) * r;
        oz = 2050 + Math.sin(a) * r;
      } else {
        const a = ((i - 26) / 10) * Math.PI;
        const r = 120 + ((i * 31) % 90);
        ox = 20 + Math.cos(a) * r;
        oz = 2320 + Math.sin(a) * r;
      }
      const oy = typeof terrainHeight === 'function' ? terrainHeight(ox, oz) : -10.0;
      const s = 0.8 + ((i * 13) % 10) * 0.08;
      dummy.position.set(ox, oy - 0.2, oz);
      dummy.rotation.set(0, (i * 1.1) % (Math.PI * 2), (i % 3 - 1) * 0.08);
      dummy.scale.set(s, s * 1.2, s);
      dummy.updateMatrix();
      obeliskMesh.setMatrixAt(i, dummy.matrix);
    }
    obeliskMesh.instanceMatrix.needsUpdate = true;
    uwGroup.add(obeliskMesh);

    // 5. Vibrant Coral Reef Formations (Living Coral Pink & Seafoam Turquoise heads)
    // Modeled after veo_underwater_coral_reef.jpg off Kaya Island
    const coralCount = 48;
    const coralDomeGeo = new THREE.DodecahedronGeometry(2.4, 1);
    coralDomeGeo.scale(1.2, 0.75, 1.2);
    const coralMat1 = new THREE.MeshStandardMaterial({
      color: 0xe86377,
      roughness: 0.6,
      emissive: 0x4a111b,
      emissiveIntensity: 0.35
    });
    const coralMesh1 = new THREE.InstancedMesh(coralDomeGeo, coralMat1, coralCount);
    ...
    const coralMesh2 = new THREE.InstancedMesh(coralDomeGeo, coralMat2, coralCount);

    for (let i = 0; i < coralCount; i++) {
      const a = (i / coralCount) * Math.PI * 2;
      const r = 55 + ((i * 43) % 140);
      const cx = 20 + Math.cos(a) * r;
      const cz = 2080 + Math.sin(a) * r;
      const cy = typeof terrainHeight === 'function' ? terrainHeight(cx, cz) : -8.0;
      const s = 0.7 + (i % 7) * 0.18;

      dummy.position.set(cx, cy + 0.5, cz);
      dummy.rotation.set((i % 4) * 0.1, (i * 0.8) % (Math.PI * 2), (i % 5) * 0.1);
      dummy.scale.set(s, s, s);
      dummy.updateMatrix();
      if (i % 2 === 0) {
        coralMesh1.setMatrixAt(i, dummy.matrix);
      } else {
        coralMesh2.setMatrixAt(i, dummy.matrix);
      }
    }
    coralMesh1.instanceMatrix.needsUpdate = true;
    coralMesh2.instanceMatrix.needsUpdate = true;
    uwGroup.add(coralMesh1);
    uwGroup.add(coralMesh2);

    // 6. Deep Ocean Abyssal Volcanic Spires (Bioluminescent Trench)
    // Modeled after veo_underwater_abyssal_trench.jpg
    const spireCount = 18;
    const spireGeo = new THREE.ConeGeometry(3.5, 32, 7);
    spireGeo.translate(0, 16, 0);
    ...
    const spireMesh = new THREE.InstancedMesh(spireGeo, spireMat, spireCount);

    for (let i = 0; i < spireCount; i++) {
      const a = (i / spireCount) * Math.PI * 1.6 - 0.8;
      const r = 160 + ((i * 29) % 180);
      const sx = 20 + Math.cos(a) * r;
      const sz = 2360 + Math.sin(a) * r;
      const sy = typeof terrainHeight === 'function' ? terrainHeight(sx, sz) : -45.0;
      const s = 0.9 + (i % 5) * 0.3;

      dummy.position.set(sx, sy - 2.0, sz);
      dummy.rotation.set((i % 3 - 1) * 0.08, i * 0.7, (i % 2) * 0.05);
      dummy.scale.set(s, s * 1.4, s);
      dummy.updateMatrix();
      spireMesh.setMatrixAt(i, dummy.matrix);
    }
    spireMesh.instanceMatrix.needsUpdate = true;
    uwGroup.add(spireMesh);

    // 7. Cataract Plunge Pool Effervescent Bubble Column
    // Directly beneath the 182-meter waterfall at (0, -360)
    const plungeBubbleCount = 350;
    const pbGeo = new THREE.BufferGeometry();
    const pbPos = new Float32Array(plungeBubbleCount * 3);
    const pbPhase = new Float32Array(plungeBubbleCount);
    for (let i = 0; i < plungeBubbleCount; i++) {
      pbPos[i * 3] = (Math.random() - 0.5) * 35;
      pbPos[i * 3 + 1] = Math.random() * 18 - 8;
      pbPos[i * 3 + 2] = -360 + (Math.random() - 0.5) * 35;
      pbPhase[i] = Math.random() * Math.PI * 2;
    }
    pbGeo.setAttribute('position', new THREE.BufferAttribute(pbPos, 3));
    pbGeo.setAttribute('aPhase', new THREE.BufferAttribute(pbPhase, 1));
    const pbMat = new THREE.PointsMaterial({
      color: 0xd6f7ff,
      size: 1.8,
      transparent: true,
      opacity: 0.65,
    ...
    const plungeBubbles = new THREE.Points(pbGeo, pbMat);
    plungeBubbles.frustumCulled = false;
    uwGroup.add(plungeBubbles);
...
//     driftwoodMat.color.setHex(0xb8aa96);
  ...
  _celestialMotes() {
    ...
    this.moteMat = new THREE.ShaderMaterial({
      transparent: true, logarithmicDepthBuffer: true, depthWrite: false,
      fog: false,
      blending: THREE.AdditiveBlending,
      uniforms: {
...
//     if (palmBark && palmBark.color) palmBark.color.setHex(0xa88e6e);
  ...
  _desertPhantasmTree() {
    const g = new THREE.Group();
    const px = -460, pz = 340;
    const py = terrainHeight(px, pz); // Base desert terrain elevation (~23m)
    g.position.set(px, py, pz);

    // Materials
    const sandstone = Surfaces.weatheredConcrete(2.5);
    const mesaTop = Surfaces.flagstone(3.0);
    const darkWood = Surfaces.bark(1.8);
    const gold = Surfaces.celestialGold(1.0);

    // Ethereal Bioluminescent Crystal Canopy Material (Transmission + Emissive Violet)
    const crystalViolet = new THREE.MeshPhysicalMaterial({
      color: 0xba68c8,
      emissive: 0x9c27b0,
    ...
    const crystalCyan = new THREE.MeshPhysicalMaterial({
      color: 0x80deea,
      emissive: 0x00e5ff,
    ...
    const crystalGold = new THREE.MeshPhysicalMaterial({
      color: 0xffe082,
      emissive: 0xffd54f,
    ...
    const phantasmBark = new THREE.MeshPhysicalMaterial({
      color: 0x3d3248, // Ancient weathered violet-silver driftwood bark
      roughness: 0.52,
      metalness: 0.24,
      clearcoat: 0.35,
    ...
    // 1. Terraced Red Sandstone Mesa Promontory (Tiered Canyon Citadel)
    // Tier 1: Base Canyon Mesa Buttress
    const t1Geo = new THREE.CylinderGeometry(34, 40, 10, 32);
    const t1 = new THREE.Mesh(t1Geo, sandstone);
    t1.position.y = 5.0;
    t1.receiveShadow = t1.castShadow = true;
    g.add(t1);

    // Tier 2: Mid Promontory Shelf (Hosts Ancestral Sun-Kiva)
    const t2Geo = new THREE.CylinderGeometry(26, 32, 8, 32);
    const t2 = new THREE.Mesh(t2Geo, sandstone);
    t2.position.y = 14.0;
    t2.receiveShadow = t2.castShadow = true;
    g.add(t2);

    // Tier 3: Summit Pinnacle Mesa (Root Foundation for Phantasm Tree)
    const t3Geo = new THREE.CylinderGeometry(18, 24, 7, 32);
    const t3 = new THREE.Mesh(t3Geo, mesaTop);
    t3.position.y = 21.5;
    t3.receiveShadow = t3.castShadow = true;
    g.add(t3);

    // 2. Ancestral Pueblo Sun-Kiva on Mid Shelf (x: 22, z: 8, y: 18.0)
    const kivaGroup = new THREE.Group();
    kivaGroup.position.set(22, 18.0, 8);

    // Sunken stone ring wall
    const kivaWall = new THREE.Mesh(new THREE.CylinderGeometry(8.5, 8.5, 2.6, 24, 1, true), sandstone);
    kivaWall.position.y = 1.3;
    kivaWall.castShadow = kivaWall.receiveShadow = true;
    kivaGroup.add(kivaWall);

    // Kiva floor flagstone
    const kivaFloor = new THREE.Mesh(new THREE.CircleGeometry(8.4, 24), mesaTop);
    kivaFloor.rotation.x = -Math.PI / 2;
    kivaFloor.position.y = 0.05;
    kivaFloor.receiveShadow = true;
    kivaGroup.add(kivaFloor);

    // Radial timber ceiling vigas (beams)
    for (let i = 0; i < 8; i++) {
      const ang = (i / 8) * Math.PI * 2;
      const viga = new THREE.Mesh(new THREE.CylinderGeometry(0.22, 0.22, 17, 8), darkWood);
    ...
    // Central Sacred Hearth Pit & Amber Ember Fire Light
    const hearthRing = new THREE.Mesh(new THREE.TorusGeometry(1.4, 0.35, 12, 24), sandstone);
    hearthRing.rotation.x = Math.PI / 2;
    hearthRing.position.y = 0.3;
    kivaGroup.add(hearthRing);

    const hearthGlow = new THREE.Mesh(new THREE.CircleGeometry(1.2, 16), new THREE.MeshBasicMaterial({ color: 0xff6600 }));
    hearthGlow.rotation.x = -Math.PI / 2;
    hearthGlow.position.y = 0.32;
    kivaGroup.add(hearthGlow);

    const hearthLight = new THREE.PointLight(0xff7722, 1.8, 35, 1.2);
    hearthLight.position.set(0, 1.2, 0);
    kivaGroup.add(hearthLight);

    g.add(kivaGroup);

    // 3. The Mystical Phantasm Tree (Summit Plateau y = 25.0)
    const treeGroup = new THREE.Group();
    treeGroup.position.set(0, 25.0, 0);

    // Gnarled, buttressed spiraling trunk
    const trunkSegments = 24, trunkHeightSegs = 20;
    const trunkH = 22.0;
    const trunkGeo = new THREE.CylinderGeometry(2.2, 5.8, trunkH, trunkSegments, trunkHeightSegs);
    const pos = trunkGeo.attributes.position;
    for (let i = 0; i < pos.count; i++) {
      const px = pos.getX(i), py = pos.getY(i), pz = pos.getZ(i);
      const hFrac = (py + trunkH * 0.5) / trunkH; // 0 to 1
      const angle = Math.atan2(pz, px);
      const rad = Math.hypot(px, pz);
      // Fluted buttress ridges (7 organic ribs) flaring down into canyon cliff
      const rib = Math.cos(angle * 7 + hFrac * 3.2) * Math.pow(1.0 - hFrac, 1.6) * 3.2;
      const twist = Math.sin(hFrac * Math.PI * 1.5) * 1.8;
      const newRad = rad + rib;
      pos.setX(i, Math.cos(angle + twist * 0.15) * newRad + Math.sin(hFrac * 4.0) * 0.8);
      pos.setY(i, py + trunkH * 0.5);
      pos.setZ(i, Math.sin(angle + twist * 0.15) * newRad);
    }
    trunkGeo.computeVertexNormals();
    const trunkMesh = new THREE.Mesh(trunkGeo, phantasmBark);
    trunkMesh.castShadow = trunkMesh.receiveShadow = true;
    treeGroup.add(trunkMesh);

    // Buttress Roots Clawing Over Mesa Rim
    for (let r = 0; r < 8; r++) {
      const rAng = (r / 8) * Math.PI * 2 + 0.15;
      const rootGeo = new THREE.CylinderGeometry(0.4, 1.5, 9.0, 8);
      rootGeo.rotateZ(0.72);
      rootGeo.rotateY(rAng);
      rootGeo.translate(Math.cos(rAng) * 6.2, -1.8, Math.sin(rAng) * 6.2);
      const rootMesh = new THREE.Mesh(rootGeo, phantasmBark);
      rootMesh.castShadow = rootMesh.receiveShadow = true;
      treeGroup.add(rootMesh);
    }

    // 8 Grand Arching Cantilevered Boughs Sweeping over Canyon Abyss
    const crystalGeosViolet = [];
    const crystalGeosCyan = [];
    const crystalGeosGold = [];

    for (let b = 0; b < 8; b++) {
      const bAng = (b / 8) * Math.PI * 2 + (b % 2) * 0.3;
      const boughGroup = [];

      // Main bough fork
      const b1 = new THREE.CylinderGeometry(0.85, 1.4, 7.5, 8);
      b1.rotateZ(0.55);
      b1.rotateY(bAng);
      b1.translate(Math.cos(bAng) * 4.5, trunkH + 2.0, Math.sin(bAng) * 4.5);
      boughGroup.push(b1);

      // Mid bough curve
      const b2 = new THREE.CylinderGeometry(0.45, 0.85, 7.0, 8);
      b2.rotateZ(0.82);
      b2.rotateY(bAng + 0.22);
      b2.translate(Math.cos(bAng + 0.22) * 9.5, trunkH + 4.8, Math.sin(bAng + 0.22) * 9.5);
      boughGroup.push(b2);

      // Outer twig
      const b3 = new THREE.CylinderGeometry(0.2, 0.45, 6.0, 6);
      b3.rotateZ(1.05);
      b3.rotateY(bAng + 0.45);
      b3.translate(Math.cos(bAng + 0.45) * 14.0, trunkH + 6.2, Math.sin(bAng + 0.45) * 14.0);
      boughGroup.push(b3);

      const mergedBough = safeMerge(boughGroup, false) || b1;
      const bm = new THREE.Mesh(mergedBough, phantasmBark);
      bm.castShadow = true;
      treeGroup.add(bm);

      // Clustered Bioluminescent Crystal Foliage Clusters on each bough
      const clusterCenter = new THREE.Vector3(Math.cos(bAng + 0.3) * 12.5, trunkH + 5.5, Math.sin(bAng + 0.3) * 12.5);
      for (let k = 0; k < 6; k++) {
        const kx = clusterCenter.x + (Math.sin(k * 2.1) * 3.5);
        const ky = clusterCenter.y + (Math.cos(k * 1.7) * 2.2);
        const kz = clusterCenter.z + (Math.sin(k * 3.4) * 3.5);
        const cRad = 1.4 + (k % 3) * 0.7;

        const cGeo = new THREE.DodecahedronGeometry(cRad, 1);
        cGeo.translate(kx, ky, kz);

        if (k % 3 === 0) crystalGeosCyan.push(cGeo);
        else if (k % 3 === 1) crystalGeosGold.push(cGeo);
        else crystalGeosViolet.push(cGeo);
      }
    }

    // Crown Peak Crystal Cluster
    for (let p = 0; p < 7; p++) {
      const px = Math.sin(p * 1.5) * 3.0;
      const py = trunkH + 5.0 + Math.cos(p * 1.2) * 2.5;
      const pz = Math.cos(p * 1.5) * 3.0;
      const crownGeo = new THREE.IcosahedronGeometry(2.4, 1);
      crownGeo.translate(px, py, pz);
      crystalGeosViolet.push(crownGeo);
    }

    if (crystalGeosViolet.length > 0) {
      const mergedViolet = safeMerge(crystalGeosViolet, false);
      if (mergedViolet) {
        const vm = new THREE.Mesh(mergedViolet, crystalViolet);
        vm.castShadow = false;
        treeGroup.add(vm);
      }
    }
    if (crystalGeosCyan.length > 0) {
      const mergedCyan = safeMerge(crystalGeosCyan, false);
      if (mergedCyan) {
        const cm = new THREE.Mesh(mergedCyan, crystalCyan);
        cm.castShadow = false;
        treeGroup.add(cm);
      }
    }
    if (crystalGeosGold.length > 0) {
      const mergedGold = safeMerge(crystalGeosGold, false);
      if (mergedGold) {
        const gm = new THREE.Mesh(mergedGold, crystalGold);
        gm.castShadow = false;
        treeGroup.add(gm);
      }
    }

    // 4. Mystical Tree Core Lighting
    this._phantasmTreeLight = new THREE.PointLight(0xba68c8, 3.4, 110, 1.2);
    this._phantasmTreeLight.position.set(0, trunkH + 4.0, 0);
    treeGroup.add(this._phantasmTreeLight);

    const cyanAccentLight = new THREE.PointLight(0x00e5ff, 2.0, 75, 1.4);
    cyanAccentLight.position.set(0, trunkH + 8.0, 0);
    treeGroup.add(cyanAccentLight);

    g.add(treeGroup);

    // 5. Drifting Spirit Motes (Bioluminescent Swirling Particle Cloud)
    const MOTE_COUNT = 85;
    const mPos = new Float32Array(MOTE_COUNT * 3);
    const mCol = new Float32Array(MOTE_COUNT * 3);
    const mSiz = new Float32Array(MOTE_COUNT);
    const mRng = mulberry32(99128);
    const tmpCol = new THREE.Color();

    for (let i = 0; i < MOTE_COUNT; i++) {
      const rad = 4.0 + mRng() * 22.0;
      const a = mRng() * Math.PI * 2;
      mPos[i * 3] = Math.cos(a) * rad;
      mPos[i * 3 + 1] = 22.0 + mRng() * 26.0;
      mPos[i * 3 + 2] = Math.sin(a) * rad;

      const paletteChoice = mRng();
      if (paletteChoice < 0.5) tmpCol.setHex(0xba68c8);
      else if (paletteChoice < 0.8) tmpCol.setHex(0x80deea);
      else tmpCol.setHex(0xffd54f);
    ...
    moteGeo.setAttribute('size', new THREE.BufferAttribute(mSiz, 1));

    this.phantasmMoteMat = new THREE.ShaderMaterial({
      transparent: true,
      depthWrite: false,
      blending: THREE.AdditiveBlending,
      uniforms: {
    ...
    const motes = new THREE.Points(moteGeo, this.phantasmMoteMat);
    g.add(motes);
  ...
  _buildUniversalStructure(cx, cz, idPrefix, plotName) {
    const cy = typeof terrainHeight === 'function' ? terrainHeight(cx, cz) : 0;
    const group = new THREE.Group();
    group.position.set(cx, cy, cz);
    this.world.scene.add(group);

    // --- High-Fidelity Procedural Materials ---
    const matSandstone = material('hohenzollernSandstone', { repeat: 6.0, color: 0xd8b571, roughness: 0.85, metalness: 0.0, normalScale: 2.0, aoMapIntensity: 1.2 });
    const matSlate = material('darkSlate', { repeat: 4.0, color: 0x2c2e33, roughness: 0.7, metalness: 0.1, normalScale: 1.0 });
    const matRedMarble = material('asamRedMarble', { repeat: 3.0, color: 0x5a1811, roughness: 0.1, metalness: 0.0, clearcoat: 0.8, clearcoatRoughness: 0.15 });
    const matGold = Surfaces.celestialGold(1.0);
    const matFresco = Surfaces.byzantineMosaic(8.0);
    const matGlass = Surfaces.clearGlass(1.0);

    

    // --- Exterior Geometry (Hohenzollern) ---
    const sandstoneGeos = [];
    const slateGeos = [];

    // 1. Bastion Terraces (Base)
    const baseGeo1 = new THREE.BoxGeometry(100, 15, 120);
    baseGeo1.translate(0, 7.5, 0);
    sandstoneGeos.push(baseGeo1);

    const baseGeo2 = new THREE.BoxGeometry(85, 12, 100);
    baseGeo2.translate(0, 21, 0);
    sandstoneGeos.push(baseGeo2);

    // 2. Main Keep
    const keepGeo = new THREE.BoxGeometry(50, 45, 70);
    keepGeo.translate(0, 49.5, 0);
    sandstoneGeos.push(keepGeo);

    // Main Keep Roof
    const keepRoofGeo = new THREE.ConeGeometry(38, 25, 4);
    keepRoofGeo.rotateY(Math.PI / 4); // align to box
    keepRoofGeo.translate(0, 84.5, 0);
    slateGeos.push(keepRoofGeo);

    // 3. Turrets and Towers
    const towerPositions = [
        {x: -25, z: 35, r: 8, h: 65, rh: 30},
        {x: 25, z: 35, r: 8, h: 65, rh: 30},
        {x: -25, z: -35, r: 8, h: 65, rh: 30},
        {x: 25, z: -35, r: 8, h: 65, rh: 30},
        {x: -42, z: 50, r: 6, h: 45, rh: 20},
        {x: 42, z: 50, r: 6, h: 45, rh: 20},
        {x: -42, z: -50, r: 6, h: 45, rh: 20},
        {x: 42, z: -50, r: 6, h: 45, rh: 20},
        {x: 0, z: 50, r: 10, h: 55, rh: 35}, // Front gate tower
        {x: 0, z: -10, r: 12, h: 90, rh: 45}  // Central high spire
    ];

    towerPositions.forEach(pos => {
        const tower = new THREE.CylinderGeometry(pos.r, pos.r + 1, pos.h, 12);
        tower.translate(pos.x, 27 + pos.h/2, pos.z);
        sandstoneGeos.push(tower);

        const roof = new THREE.ConeGeometry(pos.r + 1.5, pos.rh, 12);
        roof.translate(pos.x, 27 + pos.h + pos.rh/2, pos.z);
        slateGeos.push(roof);
    });

    if (safeMerge) {
        if (sandstoneGeos.length > 0) { const m1 = new THREE.Mesh(safeMerge(sandstoneGeos), matSandstone); m1.castShadow=true; m1.receiveShadow=true; group.add(m1); }
        if (slateGeos.length > 0) { const m2 = new THREE.Mesh(safeMerge(slateGeos), matSlate); m2.castShadow=true; m2.receiveShadow=true; group.add(m2); }
    } else {
        sandstoneGeos.forEach(geo => { const m = new THREE.Mesh(geo, matSandstone); m.castShadow = true; m.receiveShadow = true; group.add(m); });
        slateGeos.forEach(geo => { const m = new THREE.Mesh(geo, matSlate); m.castShadow = true; m.receiveShadow = true; group.add(m); });
    }

    // --- Interior Geometry (Asamkirche) ---
    const marbleGeos = [];
    const goldGeos = [];

    // Solomonic (twisted) columns simulation using stacked rotated boxes or cylinders
    for (let z = -20; z <= 20; z += 10) {
        for (let x of [-15, 15]) {
            // Twisted column effect
            for(let y = 30; y < 65; y += 2) {
                const colGeo = new THREE.CylinderGeometry(1.5, 1.5, 2, 8);
                colGeo.translate(Math.sin(y*0.5)*0.5, 0, Math.cos(y*0.5)*0.5); // twist offset
                colGeo.translate(x, y, z);
                marbleGeos.push(colGeo);
            }

            // Elaborate Gold Capitals
            const cap1 = new THREE.BoxGeometry(4, 1.5, 4);
            cap1.translate(x, 66, z);
            goldGeos.push(cap1);
            const cap2 = new THREE.CylinderGeometry(2.5, 2.5, 2, 8);
            cap2.translate(x, 67.5, z);
            goldGeos.push(cap2);
        }
    }

    // Gold Balconies
    const balcGeo1 = new THREE.BoxGeometry(32, 2, 50);
    balcGeo1.translate(0, 45, 0);
    // Hollow out center
    const innerVoid = new THREE.BoxGeometry(26, 4, 44);
    innerVoid.translate(0, 45, 0);
    // We can't CSG easily here, so we build it from 4 thin boxes
    const bLeft = new THREE.BoxGeometry(3, 2, 50); bLeft.translate(-14.5, 45, 0);
    const bRight = new THREE.BoxGeometry(3, 2, 50); bRight.translate(14.5, 45, 0);
    const bFront = new THREE.BoxGeometry(26, 2, 3); bFront.translate(0, 45, 23.5);
    const bBack = new THREE.BoxGeometry(26, 2, 3); bBack.translate(0, 45, -23.5);
    goldGeos.push(bLeft, bRight, bFront, bBack);

    if (safeMerge) {
        if (marbleGeos.length > 0) { const m3 = new THREE.Mesh(safeMerge(marbleGeos), matRedMarble); m3.castShadow=true; m3.receiveShadow=true; group.add(m3); }
        if (goldGeos.length > 0) { const m4 = new THREE.Mesh(safeMerge(goldGeos), matGold); m4.castShadow=true; m4.receiveShadow=true; group.add(m4); }
    } else {
        marbleGeos.forEach(geo => { const m = new THREE.Mesh(geo, matRedMarble); m.castShadow = true; m.receiveShadow = true; group.add(m); });
        goldGeos.forEach(geo => { const m = new THREE.Mesh(geo, matGold); m.castShadow = true; m.receiveShadow = true; group.add(m); });
    }

    // Vaulted Fresco Ceiling
    const ceilingGeo = new THREE.CylinderGeometry(15, 15, 50, 32, 1, true, 0, Math.PI);
    ceilingGeo.rotateZ(Math.PI / 2);
    ceilingGeo.rotateY(Math.PI / 2);
    ceilingGeo.translate(0, 68, 0);
    const ceilingMesh = new THREE.Mesh(ceilingGeo, matFresco); ceilingMesh.castShadow=true; ceilingMesh.receiveShadow=true;
    // Asamkirche ceiling is quite dark and dramatic
    group.add(ceilingMesh);

    // Chiaroscuro Lighting (Dramatic contrast)
    const ambientLight = new THREE.AmbientLight(0x201010, 0.8); 
    group.add(ambientLight);

    const sunBeam = new THREE.SpotLight(0xfff0dd, 25);
    sunBeam.position.set(0, 90, 0);
    sunBeam.target.position.set(0, 30, 0);
    sunBeam.angle = Math.PI / 5;
    sunBeam.penumbra = 0.8;
    sunBeam.castShadow = true;
    group.add(sunBeam);
    group.add(sunBeam.target);

    const warmGlow = new THREE.PointLight(0xffaa55, 10, 50);
    warmGlow.position.set(0, 40, -15);
    group.add(warmGlow);
  }

  _universalCathedral() {
    this._buildUniversalStructure(WORLD.cathedral.x, WORLD.cathedral.z, 'CATHEDRAL', 'Cathedral');
  }

  _buddhistPagoda() {
    this._buildUniversalStructure(WORLD.buddhistTemple.x, WORLD.buddhistTemple.z, 'PAGODA', 'Pagoda');
  }
  ...
  _moorishMosque() {
    ...
    const stone = Surfaces.weatheredConcrete(3.0);
...
//     gold.clearcoatRoughness = 0.1;
    ...
    const brass = Surfaces.brushedMetal(2.0);
...
//     darkBronze.metalness = 0.88;
    ...
    // 6. Omni-Azimuthal Volumetric Rainbow Arcs (Maintained in 100% Photoreal Radiance from ALL Viewpoints)
    // Primary Spectral Arc (Red 650nm -> Violet 400nm)
    const arcGroup = new THREE.Group();
    arcGroup.position.set(bx, 2, bz);

    // 6A. Longitudinal Z-offset planar rings
    for (let offset = -24; offset <= 24; offset += 4) {
      const arc = this._makeRainbowArc(95, 145, 0.22 * (1.0 - Math.abs(offset)/32.0), false);
      arc.position.z = offset;
      arcGroup.add(arc);
      this._rainbowShaders.push(arc.material);
    }

    // 6B. Radial cross-planes rotated around vertical axis Y (Omni-Azimuthal visibility from East, West, & 360 deg)
    const crossAngles = [-75, -60, -45, -30, -15, 15, 30, 45, 60, 75, 90, -90];
    for (const deg of crossAngles) {
      const rad = (deg * Math.PI) / 180;
      const cArc = this._makeRainbowArc(95, 145, 0.16 * Math.cos(rad * 0.4), false);
      cArc.rotation.y = rad;
      arcGroup.add(cArc);
      this._rainbowShaders.push(cArc.material);
    }

    // 6C. Continuous 3D Volumetric Arched Canopies (Semi-cylindrical arched ribbons spanning across depth)
    const primaryRibbonRadii = [98, 106, 114, 122, 130, 138, 144];
    for (const r of primaryRibbonRadii) {
      const ribGeo = this._createArchedRibbon(r, 44, 120, 4);
      const ribArc = this._makeRainbowArc(95, 145, 0.14, false, ribGeo);
      arcGroup.add(ribArc);
      this._rainbowShaders.push(ribArc.material);
    }
    g.add(arcGroup);

    // Secondary Outer Rainbow Arc (physics-accurate Alexander's Dark Band inverted spectrum)
    const arc2Group = new THREE.Group();
    arc2Group.position.set(bx, 2, bz - 6);

    for (let offset = -16; offset <= 16; offset += 4) {
      const arc2 = this._makeRainbowArc(150, 180, 0.08 * (1.0 - Math.abs(offset)/22.0), true);
      arc2.position.z = offset;
      arc2Group.add(arc2);
      this._rainbowShaders.push(arc2.material);
    }
    for (const deg of [-60, -35, 0, 35, 60, 90]) {
      const rad = (deg * Math.PI) / 180;
      const cArc2 = this._makeRainbowArc(150, 180, 0.065, true);
      cArc2.rotation.y = rad;
      arc2Group.add(cArc2);
      this._rainbowShaders.push(cArc2.material);
    }
    for (const r of [158, 172]) {
      const ribGeo2 = this._createArchedRibbon(r, 34, 100, 3);
      const ribArc2 = this._makeRainbowArc(150, 180, 0.05, true, ribGeo2);
      arc2Group.add(ribArc2);
      this._rainbowShaders.push(ribArc2.material);
    }
    g.add(arc2Group);

    // Atmospheric Sunburst Halo & Multi-Angle Core Bloom
    for (const haloRot of [0, Math.PI * 0.25, Math.PI * 0.5]) {
      const glow = this._makeRainbowArc(85, 155, 0.045, false);
      glow.position.set(bx, 2, bz - 0.5);
      glow.rotation.y = haloRot;
      g.add(glow);
    ...
    // 7. Gothic Pointed-Arch Tracery Spandrels & Iridescent Bridge Span
    // (Bespoke sculpted Cathedral Viaduct architecture matching veo_rainbow_bridge_1788571479328.jpg)
    const iridescentRainbowMat = new THREE.MeshPhysicalMaterial({
      color: 0xf8fbff,
      emissive: 0x1a2836,
      emissiveIntensity: 0.75,
      roughness: 0.10,
    ...
    const gothicTraceryGeos = [];
    const crystalLancetGeos = [];
    const NUM_GOTHIC_BAYS = 10;
    for (const s of [-16.2, 16.2]) {
      for (let bay = 0; bay < NUM_GOTHIC_BAYS; bay++) {
        const u0 = (bay + 0.05) / NUM_GOTHIC_BAYS;
        const u1 = (bay + 0.95) / NUM_GOTHIC_BAYS;
        const z0 = bz - 54 + u0 * 108;
        const z1 = bz - 54 + u1 * 108;
        const zMid = (z0 + z1) * 0.5;
        const deckTop = this.world._deckY(zMid);
        const archBottom = WORLD.waterLevel + 1.2;
        const bayHeight = Math.max(2.5, deckTop - archBottom);

        // Gothic Lancet Colonnettes
        for (const cz of [z0, z1]) {
          const colGeo = new THREE.CylinderGeometry(0.38, 0.46, bayHeight, 8);
          colGeo.translate(bx + s, archBottom + bayHeight * 0.5, cz);
          gothicTraceryGeos.push(colGeo);
        }

        // Pointed Arch Rib Head
        const bayWidth = Math.abs(z1 - z0);
        const archRadius = bayWidth * 0.52;
        const pointedArch = new THREE.TorusGeometry(archRadius, 0.30, 6, 12, Math.PI);
        pointedArch.rotateY(Math.PI / 2);
        pointedArch.translate(bx + s, deckTop - 0.8, zMid);
        gothicTraceryGeos.push(pointedArch);

        // Iridescent Crystalline Tracery Panel Infill
        const panelGeo = new THREE.PlaneGeometry(bayWidth * 0.88, bayHeight * 0.75);
        panelGeo.rotateY(Math.PI / 2);
        panelGeo.translate(bx + s, archBottom + bayHeight * 0.45, zMid);
        crystalLancetGeos.push(panelGeo);
      }
    }

    const mergedGothicTracery = mergeGeometries(gothicTraceryGeos, false);
    if (mergedGothicTracery) {
      const gMesh = new THREE.Mesh(mergedGothicTracery, marble);
      gMesh.castShadow = gMesh.receiveShadow = true;
      gMesh.frustumCulled = false;
      g.add(gMesh);
    }

    const mergedCrystalLancets = mergeGeometries(crystalLancetGeos, false);
    if (mergedCrystalLancets) {
      const lMesh = new THREE.Mesh(mergedCrystalLancets, iridescentRainbowMat);
      lMesh.frustumCulled = false;
      g.add(lMesh);
    }

    // 8. Shimmering River Water Rainbow Reflection (Mirrored Spectral Caustic Bloom)
    const waterReflect = this._makeRainbowWaterReflection(bx, bz);
    g.add(waterReflect);

    // 9. Frustum Culling Elimination across the entire Bridge System
    g.traverse(child => {
  ...
  _createArchedRibbon(r, widthZ, segments = 120, zSegs = 4) {
    const positions = [];
    const uvs = [];
    const indices = [];
  ...
  _makeRainbowWaterReflection(bx, bz) {
    ...
    const mesh = new THREE.Mesh(geo, mat);
    mesh.position.set(bx, WORLD.waterLevel + 0.08, bz);
    mesh.frustumCulled = false;
    this._rainbowWaterShader = mat;
    this._rainbowShaders.push(mat);
    return mesh;
  }

  _makeRainbowArc(r0, r1, baseOpacity, isSecondary = false, customGeo = null) {
    const geo = customGeo || new THREE.RingGeometry(r0, r1, 160, 12, 0, Math.PI);
    const mat = new THREE.ShaderMaterial({
      transparent: true, logarithmicDepthBuffer: true, depthWrite: false,
      side: THREE.DoubleSide,
      blending: THREE.AdditiveBlending, fog: false,
