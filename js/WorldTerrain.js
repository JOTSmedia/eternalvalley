...
_universalCathedral() {
    ...
    const cy = typeof this.terrainHeight === 'function' ? this.terrainHeight(cx, cz) : 0;

    const cathedralGroup = new THREE.Group();
    cathedralGroup.position.set(cx, cy, cz);
    ...
    const sandstoneDiff = createTexture(1024, 1024, (ctx, w, h) => {
    ...
    const sandstoneRough = createTexture(1024, 1024, (ctx, w, h) => {
    ...
    // 2. Gold / Brushed Metal
    const goldDiff = createTexture(1024, 1024, (ctx, w, h) => {
    ...
    // 3. Dark Red/Brown Veined Marble
    const marbleDiff = createTexture(1024, 1024, (ctx, w, h) => {
    ...
    // 4. Fresco (Ceiling)
    const frescoDiff = createTexture(1024, 1024, (ctx, w, h) => {
        ...
        for(let i=0; i<10; i++) {
            ctx.beginPath();
            ctx.arc(Math.random()*w, Math.random()*h, 20, 0, Math.PI*2);
            ctx.fill();
        }
    });

    // --- Materials ---
    const matSandstone = new THREE.MeshPhysicalMaterial({
        map: sandstoneDiff,
        roughnessMap: sandstoneRough,
        roughness: 0.9,
        metalness: 0.1
    });
    ...
    const matMarble = new THREE.MeshPhysicalMaterial({
        map: marbleDiff,
        roughness: 0.1,
        metalness: 0.0,
        clearcoat: 1.0,
    ...
    const matFresco = new THREE.MeshPhysicalMaterial({
        map: frescoDiff,
        roughness: 1.0,
        metalness: 0.0,
        side: THREE.DoubleSide
    ...
    const exteriorGeometries = [];
    
    // Main Body
    const bodyGeo = new THREE.BoxGeometry(60, 40, 80);
    bodyGeo.translate(0, 20, 0);
    exteriorGeometries.push(bodyGeo);
    ...
    towerPositions.forEach(pos => {
        // Cylindrical Tower
        const tower = new THREE.CylinderGeometry(8, 10, 60, 16);
        tower.translate(pos[0], 30, pos[1]);
        exteriorGeometries.push(tower);

        // Conical Roof
        const roof = new THREE.ConeGeometry(10, 20, 16);
        roof.translate(pos[0], 70, pos[1]);
        exteriorGeometries.push(roof);
    });

    // Optional: Using THREE.BufferGeometryUtils (must be imported in environment)
    if (typeof THREE.BufferGeometryUtils !== 'undefined') {
        const exteriorGeo = THREE.BufferGeometryUtils.mergeGeometries(exteriorGeometries);
        const exteriorMesh = new THREE.Mesh(exteriorGeo, matSandstone);
        cathedralGroup.add(exteriorMesh);
    } else {
        // Fallback if not available directly
        exteriorGeometries.forEach(geo => {
            const mesh = new THREE.Mesh(geo, matSandstone);
            cathedralGroup.add(mesh);
        });
    }

    // --- Interior Geometry (Asamkirche) ---
    const marbleGeometries = [];
    const goldGeometries = [];

    // Columns
    for (let z = -30; z <= 30; z += 15) {
        for (let x of [-20, 20]) {
            const colGeo = new THREE.CylinderGeometry(2, 2, 35, 16);
            colGeo.translate(x, 17.5, z);
            marbleGeometries.push(colGeo);

            // Gold capitals
            const capGeo = new THREE.BoxGeometry(5, 2, 5);
            capGeo.translate(x, 36, z);
            goldGeometries.push(capGeo);
        }
    }

    if (typeof THREE.BufferGeometryUtils !== 'undefined') {
        if (marbleGeometries.length > 0) {
            const marbleGeo = THREE.BufferGeometryUtils.mergeGeometries(marbleGeometries);
            cathedralGroup.add(new THREE.Mesh(marbleGeo, matMarble));
        }
        if (goldGeometries.length > 0) {
            const goldGeo = THREE.BufferGeometryUtils.mergeGeometries(goldGeometries);
            cathedralGroup.add(new THREE.Mesh(goldGeo, matGold));
        }
    } else {
        marbleGeometries.forEach(geo => cathedralGroup.add(new THREE.Mesh(geo, matMarble)));
        goldGeometries.forEach(geo => cathedralGroup.add(new THREE.Mesh(geo, matGold)));
    }

    // Fresco Ceiling (Vaulted)
    const ceilingGeo = new THREE.CylinderGeometry(25, 25, 70, 32, 1, true, 0, Math.PI);
    ceilingGeo.rotateZ(Math.PI / 2);
    ceilingGeo.rotateY(Math.PI / 2);
    ceilingGeo.translate(0, 38, 0);
    const ceilingMesh = new THREE.Mesh(ceilingGeo, matFresco);
    cathedralGroup.add(ceilingMesh);

    // Chiaroscuro Lighting
    const ambientLight = new THREE.AmbientLight(0x201010, 0.4); 
    cathedralGroup.add(ambientLight);

    const sunBeam = new THREE.SpotLight(0xfff0dd, 10);
    sunBeam.position.set(0, 60, 0);
    sunBeam.target.position.set(0, 0, 0);
    sunBeam.angle = Math.PI / 6;
    sunBeam.penumbra = 0.5;
    sunBeam.castShadow = true;
    cathedralGroup.add(sunBeam);
    cathedralGroup.add(sunBeam.target);
    ...
    plotPositions.forEach(p => {
        // Balcony / Plot Platform
        const platGeo = new THREE.BoxGeometry(18, 1, 18);
        platGeo.translate(p.x, 30, p.z);
        const platMesh = new THREE.Mesh(platGeo, matSandstone);
        cathedralGroup.add(platMesh);

        // Add plot via terrain addPlot logic
        if (typeof this.addPlot === 'function') {
            // Adjust y internally if addPlot handles height, otherwise it's just conceptual 2D
            this.addPlot(cx + p.x, cz + p.z, 20, 20, 'cathedral_interior');
        }
    });
}
...
//     cherryBark.color.setHex(0x382820);
    ...
    const pondWaterMat = new THREE.MeshPhysicalMaterial({
      color: 0x184c56, roughness: 0.05, metalness: 0.1, transparent: true, opacity: 0.9, logarithmicDepthBuffer: true, depthWrite: true,
      });
