...
export const CharityUI = {
  ...
  async renderPanel(ui) {
    const allList = await Campaigns.load();
    ...
    body.querySelector('#newCampaign').onclick = () => ui.campaignCreateModal();
    ...
    body.querySelectorAll('[data-give]').forEach(b => {
      b.onclick = async (e) => {
        e.stopPropagation();
        const c = await Campaigns.get(b.dataset.give);
        if (c) ui.campaignDonateModal(c);
      };
    });
    body.querySelectorAll('[data-cmp]').forEach(el => {
      el.onclick = async () => {
        const c = await Campaigns.get(el.dataset.cmp);
        if (c) ui.campaignViewModal(c);
      };
    });
  },
  ...
  createModal(ui) {
    ...
    box.querySelector('#cmpCreate').onclick = async () => {
      const name = box.querySelector('#cmpName').value.trim();
      if (!name) return ui.toast('Please enter their name.', 'warning');
      if (!Auth.user) await Auth.continueAsGuest(true);

      const btn = box.querySelector('#cmpCreate');
      const origText = btn.innerHTML;
      btn.innerHTML = `${icon('heart')} Creating...`;
      btn.disabled = true;
      ...
      try {
        ...
        ui.toast('Could not create campaign: ' + err.message, 'warning');
  ...
  async ledgerModal(ui) {
    ...
    document.getElementById('modalBox').querySelector('#ledgerExport').onclick = () => {
      ...
      a.download = `eternal-valley-charity-ledger-${new Date().toISOString().slice(0, 10)}.json`;
      a.click();
  ...
  _demoNote() {
    return 'Eternal Valley operates under an open-source, 100% verified charitable pass-through model. '
