...
// ============================================================
import { FIREBASE_CONFIG, IS_DEMO, IS_ADMIN } from './config.js?v=26';
...
async function getFbAuth() {
  ...
  const appMod = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-app.js?v=26');
  fbAuthMod = await import('https://www.gstatic.com/firebasejs/10.12.2/firebase-auth.js?v=26');
...
export const Auth = {
      ...
      mod.onAuthStateChanged(auth, (u) => {
        if (u) {
          ...
          try {
            ...
            console.warn('[auth] QuotaExceededError on ev_user setItem', e);
          }
        } else {
          this.user = null;
          window.USER = null;
  ...
  continueAsGuest(anonymous = true, name = '') {
    ...
    try {
      ...
      console.warn('[auth] QuotaExceededError on ev_user setItem in guest mode', e);
    }
    this._emit();
  },
  async signOut() {
  ...
  _demoLogin(name, email, provider) {
    ...
    try {
      ...
      console.warn('[auth] QuotaExceededError on ev_user setItem in demo login', e);
    }
    this._emit();
  },
};
