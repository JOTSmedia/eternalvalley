...
export class MemorialManager {
  ...
  init(occupiedPlots) {
    ...
    for (const plot of occupiedPlots) {
      ...
      if (m.videoUrl && window.videoManager) {
        map = window.videoManager.createVideoTexture('plot_' + plot.id, m.videoUrl, false);
      } else if (m.photo) {
      ...
      if (m.videoUrl && window.videoManager) {
        window.videoManager.registerMesh('plot_' + plot.id, mesh);
      }
    }
  }
}
