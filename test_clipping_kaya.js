...
for(let i=0; i<steps; i++) {
    ...
    if (intPoint >= 18 && intPoint <= 20) {
        const th = terrainHeight(pt.x, pt.z);
        if (pt.y < th) {
            console.log(`Clipping between Kaya and Panorama: pt=(${pt.x.toFixed(1)}, ${pt.y.toFixed(1)}, ${pt.z.toFixed(1)}) terrain=${th.toFixed(1)}`);
        }
    }
}
