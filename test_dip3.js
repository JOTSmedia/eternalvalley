...
const tourPoints = [
  ...
  new Vector3(-350, 240.0, -350), // 12
  new Vector3(-500, 210.0, -150), // 13
  new Vector3(-300, 120.0, -100), // 14
  new Vector3(200, 90.0, -250), // 15
  new Vector3(450, 310.0, -350), // 16
  new Vector3(580, 340.0, -550), // 17
  new Vector3(400, 220.0, 800), // 18
...
for(let i=13; i<=16; i++) {
    console.log(`\n--- Segment from index ${i} to ${i+1} ---`);
    for(let w=0; w<=1.0; w+=0.2) {
        const t = (i + w) / tourPoints.length;
        const pt = getCurvePoint(tourPoints, t);
        console.log(`w=${w.toFixed(1)}: pt=(${pt.x.toFixed(1)}, ${pt.y.toFixed(1)}, ${pt.z.toFixed(1)})`);
    }
}
